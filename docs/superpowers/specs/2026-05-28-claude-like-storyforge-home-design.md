...
本次重构目标是借鉴 Claude `/new` 的视觉骨架和低噪声交互体验，将 StoryForge 首页改造为“创作意图入口”，同时保留项目已有的可验证长篇小说创作流水线逻辑。

本设计只借鉴布局与交互轻量感，不复制 Claude 的业务逻辑。StoryForge 的核心逻辑仍围绕：
...
1. **视觉骨架接近 Claude 新建页**：深色背景、左侧导航、顶部状态胶囊、中央问候语、大输入框、胶囊快捷按钮。
...
左侧导航保留 Claude 式垂直列表，但内容替换为 StoryForge 真实入口：
...
- 首页容器使用深色背景和局部 class。
- 现有 Studio、Retrieval、Runs 等详情页暂不强制改成 Claude 风格。
