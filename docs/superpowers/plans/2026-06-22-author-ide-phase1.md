# StoryForge 作者辅助 IDE Phase 1 Plan
...
本阶段已按 Definition of Done 重新验收通过。下方 P0-P6 保留原始计划拆解；实际完成证据以本节和 `.codex/verification-report.md` 的「作者辅助 IDE Phase 1 收口（2026-06-22）」为准。
...
把 StoryForge 第一阶段收口为“面向小说项目的作者辅助 IDE”：

> 用户打开一个本地小说项目，在 Desktop IDE 中编辑稿件；Agent 能理解当前文件和项目上下文，进行审稿、解释、定向修订和续写；所有写入先生成 diff / proposed patch，用户确认后才写回本地文件，并留下版本记录。

本阶段不以 BookRun 控制台为主入口。长篇、短篇、章节和修订输出统一表达为 Writing Run / 写作任务；BookRun 只作为 managed full-book run 的内部兼容实现存在。Judge、Repair、Story Memory、导出等能力作为 Agent 可调用工具存在。

## 当前代码事实

- `apps/desktop/frontend/src/App.tsx` 已经是对话式 agent shell，包含本地项目、文件树、编辑器、Assistant 面板、布局切换和 Tauri 菜单事件。
...
- 不恢复 `apps/web` 作为主体验。
- 不把 BookRun 页面化为第一优先级；用户侧只看到写作任务。
- 不先做完整自动长篇生产闭环。
...
### Workflow 是 Writing Run 重型引擎

短任务走 inline Writing Run，长篇/整卷/整书等长任务走 managed Writing Run。BookRun 不再作为第一屏主流程，而是 managed full-book run 的内部兼容实现。
...
- [ ] 明确写入“StoryForge = 作者辅助写作 IDE”。
- [ ] 明确 `apps/desktop` 是唯一主体验。
- [ ] 明确 Writing Run 是统一输出概念，BookRun 是 managed full-book run 的内部兼容实现。
...
**目标**：file.review 成为作者辅助 IDE 的核心能力，而不是后端测试里的结构化对象。
...
**目标**：让 Agent 像理解代码库一样理解小说项目的基本结构。
...
### P6. Writing Run Managed 模式接入

**目标**：用户侧以 Writing Run / 写作任务发起长篇或整书输出；BookRun 保留为 `bookrun.start` 兼容入口和 managed full-book run 内部实现。
...
- [ ] Agent 启动 managed Writing Run 前必须说明将生成哪些章节、预算是多少。
- [ ] 高风险或长预算写作任务需要用户确认。
- [ ] 写作任务进度以 Agent tool trace 或轻量面板展示，不抢主界面。
...
6. P5：补小说项目索引 v1。
7. P6：把 managed Writing Run 接入 Agent，避免 BookRun 重新变成主入口。
...
> StoryForge 已具备作者辅助小说项目 IDE 的最小闭环：本地文件编辑、Agent 审稿、定向修订、diff 确认、真实写回和版本记录。

仍然不能宣称：

- 稳定生产级长篇自动生成闭环完成。
- 真实 3-5 万字长程质量验收通过。
- managed Writing Run 已成为成熟批量生产系统。
