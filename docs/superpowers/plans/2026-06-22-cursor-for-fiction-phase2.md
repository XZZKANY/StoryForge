# StoryForge Cursor for Fiction Phase 2 Plan
...
本阶段已按 Definition of Done 完成本地验收。下方 P0-P7 保留原始计划拆解；实际完成证据以本节和 `.codex/verification-report.md` 的「Cursor for Fiction Phase 2 收口（2026-06-23）」为准。
...
- `bookrun.start` 改为预检/确认两段式，返回章节计划、预算、风险和确认 action；确认后前端订阅 run events 并展示轻量进度。
...
Phase 1 已经证明 StoryForge 能完成 Cursor-like 小说 IDE 的最小闭环：本地项目、当前稿、Agent 审稿、定向修订、proposed patch、确认写回和版本记录。

Phase 2 的目标不是重写这条闭环，而是把它从“能跑通”推进到“像日常工具一样好用”：

> 作者在 Desktop IDE 中与 Agent 连续协作；Agent 的思考步骤、工具调用、上下文选择、审稿问题、修订范围和补丁写回都可见、可控、可回溯。BookRun 作为后台工具被 Agent 调用和展示进度，但不抢主界面。
...
- `apps/api/app/domains/ide/router.py` 已有 BookRun SSE 快照端点 `/api/ide/runs/{book_run_id}/events`，但 Desktop Agent 尚未把 BookRun 进度作为轻量工具面板接起来。
...
11. 用户要求启动 BookRun 时，Agent 先说明章节计划、预算和风险；确认后在轻量 tool trace 中显示进度。
...
Agent 可以调用 BookRun，但 UI 只展示轻量计划、预算、状态和事件，不创建新的主控制台。
...
- `docs/superpowers/plans/2026-06-22-cursor-for-fiction-phase2.md`
...
- 文档仍明确 BookRun 是 Agent tool / 后台引擎。
...
### P5. BookRun 作为 Agent 工具的轻量进度

**目标**：允许 Agent 启动 BookRun，并用 tool trace 展示计划、预算和进度，但不新增主控制台。
...
- [ ] 高风险或长预算 BookRun 需要用户二次确认。
- [ ] 成功启动后返回 `book_run_id`，前端可订阅 `/api/ide/runs/{book_run_id}/events`。
- [ ] Agent 面板展示 BookRun 轻量进度：章节数、状态、最近事件、失败原因。
- [ ] BookRun 失败不影响当前编辑器草稿和 pending patch。

**验收**：

- API 测试覆盖预算摘要、二次确认和 `book_run_id`。
- 前端 mock SSE 覆盖 BookRun progress 渲染。
...
- [ ] API 测试覆盖 Agent 事件协议、BookRun 工具计划和修订范围。
...
6. P5：最后接 BookRun 轻量进度，避免后台引擎抢主线。
...
8. BookRun 可作为 Agent tool 展示计划、预算和轻量进度，但不变成主入口。
...
> StoryForge 不只具备 Cursor-like 小说 IDE 的最小闭环，还具备可观察、可控、可追溯的 Agent 协作体验：流式步骤、显式上下文、问题队列、补丁审阅、版本/作者闭环和 BookRun 工具进度。

仍然不能宣称：

- 稳定生产级长篇自动生成质量已经通过。
- 真实 3-5 万字长程人工通读已经验收。
- BookRun 已经成为成熟主产品控制台。
