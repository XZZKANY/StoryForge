...
- 2026-07-11（第三轮过夜重构 PR #125 收口后）：`pnpm verify` 绿（API pytest 939 passed、前端 vitest 148 passed、lint / typecheck 绿）、`pnpm e2e` 契约绿、OpenAPI 零漂移；证据见 `.codex/verification-report.md` 回填记录。以下为历史记录：
...
- 完整真实 Tauri 桌面端到端：首轮门禁 G.1 已于 2026-07-07 全 PASS（Part1-4，含打开项目 -> 对话 -> 审稿 -> 修订 -> diff 确认 -> 写回 -> 版本记录的 UI 闭环）；剩余为重建 0.1.2 后的第二轮堆积观感波（壳子 redesign 新 UI、WS 子协议、SSE、IME、canon dossier、权限四轨、双开），一次捆绑 2-3h 执行。现有 smoke 已覆盖写回护栏和版本元数据，但不能替代人工桌面端到端验收。
- 编辑器「安全可日更」波（2026-07-11 拍板序列）：S7 装机前两小刀（Rust 写侧 containment、L7 单实例守卫）→ S14 尾巴 → S8 重建 0.1.2 NSIS → S9 AI 装机预验 → S10 真机第二轮观感波 → S11 修复 + 轻量锁版 tag；完成后在编辑器上接续 n=1 连载（S3 手稿保险挪到开写前夕建）。
