...
2026-07-11 编辑器优先 Phase A 已完成 A1-A5：Rust `write_file` / `create_dir` / `delete_path` / `rename_path` 全部改为显式项目根 + canonical containment（含外部路径、symlink 父目录、项目根删除 cargo 护栏）；Tauri 接 `tauri-plugin-single-instance`，第二次启动唤醒并聚焦主窗口；Agent 右栏折叠改常驻挂载 + CSS 隐藏，happy-dom 证明 state 跨隐藏保留；版本四源升至 0.1.2 并产出 NSIS/MSI。A5 已在本机覆盖安装并自动预验：packaged sidecar、0.1.2 ready 握手、严格 WS 子协议 headless 对话、SSE 终态回放、0.1.1 旧孤儿强杀替换、存量 SQLite Alembic 纳管与会话史保留均绿。A6 真机第二轮观感波与 A7 blocker 修复/tag 仍未执行，故 Gate A→B 尚未通过。
