...
StoryForge 当前处于真实长程验收整改与 Cursor for Fiction Phase 1 收口阶段。产品契约为：StoryForge = Cursor for Fiction，`apps/desktop` 是唯一主体验；`pnpm dev` / `pnpm desktop:dev` 启动桌面 IDE 主体验；`apps/web` 已退场，不再作为维护、调试、兼容或契约验证入口。

第一阶段验收链路固定为：本地文件审稿 -> 修订 -> diff 确认 -> 真实写回 -> 版本记录。BookRun 是 Agent tool / 后台重型引擎，保留长程生成、审计和导出能力，但不作为第一阶段主产品控制台。
...
- **BookRun 后台引擎 / Agent tool**：本地 deterministic/mock provider 可从 Blueprint 章节计划驱动 BookRun，并导出 `book.md` 与 `audit_report.json`；BookRun 已具备 checkpoint resume、token/时间/章节预算暂停、provider 连续降级暂停和成本摘要，但不作为主产品入口。
...
- **Desktop IDE Agent**：后端 IDE Agent Orchestrator 已支持自然语言意图路由、真实 LLM 文件修订、多视角 file.review 推理缝、稳定 issue id、范围控制、待确认 proposed patch 和确认写回防重复生成；前端已接入对话、步骤树、待确认 diff 和本地确认写回事件。
...
- Desktop frontend typecheck/unit/smoke：承接 Web 退场后的前端门禁；本轮需重新记录最新执行结果。
- `cd apps/api && uv run pytest tests/test_ide_agent_orchestrator.py -q`：最近记录通过，覆盖 IDE Agent 编排、审稿推理缝、范围控制和确认写回防御。
- `cd apps/desktop/frontend && pnpm.cmd run verify:agent-conversation`：最近记录通过，覆盖桌面 Agent 对话预览链路。
...
- 视需要补齐 WebSocket 流式响应、生产级对象存储签名下载、多租户认证和 Desktop 内的全步骤交互编排。

## 禁止宣称范围

在上述未完成项补齐前，只能宣称 StoryForge 已具备本地可验证的最小整书闭环、真实 LLM 10 章 smoke 验收证据、一次 30 章真实长程链路与制品导出证据，以及 Cursor for Fiction Phase 1 的 Desktop IDE 最小闭环验证证据；不能宣称真实 3-5 万字长程质量验收通过，也不能宣称具备稳定生产级长篇生产闭环。不得把 `apps/web` 或 BookRun 控制台描述为主产品入口。
