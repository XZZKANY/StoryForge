...
- E2E-1 真机验收首轮门禁 G.1 已于 2026-07-07 全 PASS（Part1-4 含 UI 闭环；共逮 6 真 bug 均修，PR #87/#88/#89/#95/#96/#109）；第二轮中壳子 redesign 新 UI 全流程与中文 IME 已于 2026-07-11 PASS，真机剩余 = SSE/REST、canon dossier、权限四轨、双开与运行控制。
- 编辑器优先 Phase A（2026-07-11）：A1-A5 已完成。0.1.2 NSIS 已构建并覆盖安装；packaged smoke、安装态 ready/WS/SSE（历史 A5 证据）、旧孤儿版本替换、存量库 Alembic + 会话史均预验通过。A6 壳子新 UI 已 PASS，其余观感项继续按 `.codex/e2e-2-runsheet-0.1.2.md`；A7/tag 等 A6 blocker 结论。
...
1. 编辑器做到「安全可日更」（第一段）：A1-A5 已完成；A6 壳子新 UI 全流程（含会话历史列表）与中文 IME 已 PASS。现在按 runsheet 继续跑真实 Tauri 桌面端到端剩余观感项（SSE/REST 工具循环、canon dossier、权限四轨、双开聚焦、暂停/停止、超时轮询）→ A7 只修 blocker、复验并打轻量 `v0.1.2` tag。首轮门禁 G.1 已于 2026-07-07 全 PASS；A6/A7 未完成前 Gate A→B 不通过。
2. 在编辑器上写作品（第二段）：S3 手稿保险（连载目录仓库外 git init + 计划任务自动 commit，挪到开写前夕建）→ 接续 n=1 连载（创作资产存档见「当前事实边界」末条；canon.json 首刷吃末世系统数字状态）；写作即 dogfood，摩擦日志驱动每周至多一刀 QoL。
