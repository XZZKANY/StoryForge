...
1. Agent loop 收口（余项）：真机 GUI 多轮渲染、自动打开新文件与补丁确认观感随桌面端到端复验（审稿 / 修订 / 起草 / 一致性观察 / 深度一致性的工具循环内 headless 实跑均已通过；深度一致性已于 2026-07-02 落地——`resolved_llm_env` 下沉 `app/common/llm_env.py`、语义 judge 吃 `llm-provider.json` 覆盖链、`project.deep_consistency` 挂进循环；2026-07-04 W1 已补齐 live 循环可中断 / 起服收尸 / 断线重建终态 / 关键词表下线 / sidecar 版本握手，PR #70，schema 冻结下零 ORM 变更）。W1 新行为真机验收点：点停止后事件表无后续 tool_trace、超时转后台仍取回结果、强杀宿主重启无孤儿且连新 sidecar（承接 E2E-1 清单）。
