...
- E2E-1 真机验收首轮门禁 G.1 已于 2026-07-07 全 PASS（Part1-4 含 UI 闭环；共逮 6 真 bug 均修，PR #87/#88/#89/#95/#96/#109）；真机剩余 = 第二轮堆积观感波（壳子 redesign 新 UI、WS 子协议、SSE、IME、canon dossier、权限四轨、双开），归重建 0.1.2 后一次捆绑执行。
- 编辑器优先 Phase A（2026-07-11）：A1-A5 已完成。0.1.2 NSIS 已构建并覆盖安装；packaged smoke、安装态 ready/WS/SSE、旧孤儿版本替换、存量库 Alembic + 会话史均预验通过。当前下一步仅是 A6 用户真机观感波；runsheet 在 `.codex/e2e-2-runsheet-0.1.2.md`。A7/tag 等 A6 blocker 结论。
...
1. 编辑器做到「安全可日更」（第一段）：A1-A5 已完成。现在按 runsheet **跑真实 Tauri 桌面端到端** A6 第二轮堆积观感波（一次捆绑 2-3h：壳子新 UI、工具循环流程树、会话历史列表、WS/SSE、IME、canon dossier、权限四轨、双开聚焦、暂停/停止、超时轮询）→ A7 只修 blocker、复验并打轻量 `v0.1.2` tag。首轮门禁 G.1 已于 2026-07-07 全 PASS；A6/A7 未完成前 Gate A→B 不通过。
