...
1. Agent loop 收口（余项）：真机 GUI 多轮渲染、自动打开新文件与补丁确认观感随桌面端到端复验（审稿 / 修订 / 起草 / 一致性观察 / 深度一致性的工具循环内 headless 实跑均已通过；深度一致性已于 2026-07-02 落地——`resolved_llm_env` 下沉 `app/common/llm_env.py`、语义 judge 吃 `llm-provider.json` 覆盖链、`project.deep_consistency` 挂进循环）。
