...
1. Agent loop 收口：深度一致性（Character Bible / 语义 judge）做成工具循环内工具（语义 judge 需先从 `os.getenv` 迁 `resolved_llm_env` 才能吃 `llm-provider.json`）；真机 GUI 多轮渲染、自动打开新文件与补丁确认观感随桌面端到端复验（审稿 / 修订 / 起草 / 一致性观察的循环内 headless 实跑均已通过）。
