...
1. **补齐文件边界验收。** 文件工具已实现逐目标校验和资源预算，修复尚未提交；普通符号链接三项测试受权限限制跳过。Windows junction、资源限制及冻结 exe 实际搜索已有通过证据，不重复立项为“尚未实现保护”。
2. **回到真实写作验证。** 接续 n=1 连载前确认仓库外资产现状；记录固定任务的完成时间、改稿保留率、返工和人工偏好。7 月章节数与 S3 手稿保险计划任务状态本轮未复核，不作为今天事实。
3. **按版本执行发布验收。** 指定拟发布构建，运行对应门禁并复验 GUI 保存、diff 确认和权限档位；冻结 sidecar 冒烟不能替代真机 GUI 验收。
