...
（2026-07-11 拍板重排：08-31 盛夏寻章不当锚，编辑器优先；原「窗口倒排」序列作废。）

1. 编辑器做到「安全可日更」（第一段）：S7 装机前两小刀（Rust 写侧 containment、L7 单实例守卫）→ S14 尾巴（壳子 #2 面板 unmount 改 CSS 隐藏）→ S8 重建 0.1.2 NSIS（收进 PR #87-#125 全部修复）→ S9 AI 装机预验（headless 跑绿 WS/SSE 再交真机）→ S10 跑真实 Tauri 桌面端到端第二轮堆积观感波（一次捆绑 2-3h：工具循环流程树、会话历史列表、WS 子协议、SSE、IME、canon dossier、权限四轨、双开；首轮门禁 G.1 已于 2026-07-07 全 PASS）→ S11 修复波 + 轻量锁版 tag。
