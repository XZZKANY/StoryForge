...
> 上位约束见 `AGENTS.md`；当前阶段事实以 `current-phase.md` 为准；天命解构参考见 `reference-tianming-analysis.md`；架构方向见 memory `project_agent_orchestration_direction` / `project_live_judge_chain_truth`。
...
- **回滚**（resume/重跑某章）= 删 `chapter_index > target` 的事件后 reproject。**不裁剪历史**——这避开了天命最大的规模裂缝（它截断历史导致远距召回失忆）。
...
> 白捡升级：天命用 flat 字符串比较的 V6（同章既盟友又仇敌）、V4（移动链）在我们这边变成 `edge_constraints` 图查 / 位置序列检查，**对重名、链式传递更稳**（见 `reference-tianming-analysis.md`）。

### #4 CHANGES 落法 + 骨架来源

**传输 = 工具调用**：Writer 调 `commit_chapter(prose, changes[])`，`changes` 是带类型的结构化项，**schema 在工具层校验 + 重试**——绕开天命 CHANGES 文本块的脆弱解析器。

**实体引用 = 花名册挑 id**：骨架带花名册（entity + 稳定 id + 别名）；Writer 报 CHANGES 直接引 `char_001`，不报自由文本名。新实体（花名册没有）→ Writer 标 `new` → 当场幂等铸造 id + 回写花名册。**消解前移到声明时**，天命 `name_registry` 那套事后归一的脆弱基本自动消解。
...
对照天命 12 类 CHANGES 与 V1-V8 不变量（详见 `reference-tianming-analysis.md`）：

| CHANGES 类 | 去处 | 不变量 / 检查 | 天命对应 |
|---|---|---|---|
| 关系 / 信任度Δ | **edge** relationship | 信任Δ越界；同章既盟友又仇敌（图查矛盾） | V6 |
| 时间推进 | **edge** timeline_order | 时序回退 / 成环（`WITH RECURSIVE`） | — |
| 角色等级 / 能力 / 心理 | **node** | 等级单调；能力获得须有事件 | V3 |
| 角色位置 / 移动 | **node**（位置）+ 序列 | 在场才能行动；移动链 from = 上次 to | V3/V4 |
| 伏笔 setup / payoff | **node** | 未埋先收 / 已收回退 / 逾期 | V1 |
| 冲突进度 | **node** | 阶段序 pending<active<climax<resolved | V2 |
| 地点 / 势力状态 | **node** | 状态合法迁移 | — |
| 物品流转 | **node** | 单持有人；持有链 | V5 |
| 秘密揭示 | **node**（+ 可选 edge） | 知情集只增 | — |
| 誓约 | **node** | 单终止动作；不重复 create | V7 |
| 倒计时 | **node** | 同 V7 形态 | V8 |
