...
**总体判决:方向已对,地基错位。** 产品在 2026-06-24/06-30 两次拍板后已收敛为「单机 sidecar + 对话式 agent 的作者辅助 IDE」,但代码地基仍是 web/多租户/自动整书时代的企业级架构:38 个域、45 张表、五容器云栈、四层重叠门禁——而真正的 live 产品面只有 4 条路由族。审计确认 14 条重大发现(2 critical + 12 high,全部经双重对抗核查存活),其中唯一「不动就会炸」的是 **F01:已装机 Alpha 用户的 sqlite 库零迁移机制**。
...
- **slice「权限四轨收敛」(2026-07-07 补做,E2E-1 PASS 后,PR #111/#112,两切片先绿再切)**:破当初「`requires_confirmation` 无法纯从 `risk_level` 派生」的跳过理由 —— 加上 `execution_mode` 共同作输入即可(`bookrun.pause/resume/retry` 是 `execution_mode="control"`、`bookrun.start` 是 `long_running`),经 15 spec 逐条核验规则成立:`requires_confirmation = risk∈{write_pending,high_cost,propose_patch,network} OR (risk==long_running AND mode≠control)`。**slice1(PR #111,纯 additive)**:`tooling.py` 加 `derive_requires_confirmation`/`derive_permission_level`/`confirming_tool_names` 三纯函数 + 平价元测试断言派生值逐条==当前声明值、确认集=={file.revise,file.create,judge.repair,bookrun.start}(==runtime 手写放行名单),先绿安全垫。**slice2(PR #112)**:`AgentRuntimeToolSpec` 的 `permission_level`/`requires_confirmation` 降为读派生函数的 `@property`、删 15 spec 字面量两 kwarg(消除三字段漂移);`runtime._execute_tool` 手写放行名单改 `not tool.requires_confirmation`(拆隐藏第 5 轨);`PermissionGate.decide` 删 `full_allow`/`autonomous_approval`/`step_confirm` 三不可达分支(live profile 恒 `risk_confirm`,前端从不发送),只留 `risk_confirm` + `read`/未知 fallthrough 放行。行为保持(派生值==旧声明值)、全量 884 passed、OpenAPI 零漂移。边界:只收敛 agent_runs 15 spec;internal/MCP 另一套词表已各自单点派生,不动;未拆 1401 放行本身/不改工件层写回红线。
- **未做(经用户 2026-07-07 拍板)**:①slice 4「第一个 Q 工具」**跳过** —— 从 prose 抽结构化边需已 park 的 story_state/typed-delta 方向或启发式,且真·LLM headless gate 跑不了,且与 `project.deep_consistency` 部分重叠,需产品定向。③slice 5「删 orchestrator facade」**保留 facade** —— 该 40 行 compat 非死码(live `runtime.py` noqa-import 作 monkeypatch 靶 + `test_ide_agent_orchestrator.py` 专门测试保护),删它价值低于风险。
