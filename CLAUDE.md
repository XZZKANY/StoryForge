...
- Agent loop 边界：工具循环入口是 chat 自由文本；审稿 / 修订已作为循环内工具并入（`file.review` / `file.revise`，一次对话最多一个待确认补丁），chapter.review / 写作任务等其余显式 intent 与显式按钮路径仍走固定管线；真·LLM tool-calling headless 实跑已通过（2026-07-02，deepseek-v4-flash，证据 `.codex/real-llm-agent-loop-20260702-165907` 与 `.codex/real-llm-agent-loop-intents-20260702`），真机 GUI 渲染观感未验；写回红线不变，后端不写项目文件，修订走 proposed patch 前端确认。
...
- Desktop 对话式 Agent：项目级对话会话（切文件不丢，消息持久化于 `assistant_sessions`，左栏会话历史列表可切换 / 新建）、`chat.explain` 真·LLM 回话、chat 自由文本 LLM 工具循环（path-scoped 只读 `fs.list` / `fs.read` / `fs.search`，逐调用证据链，流程树全事件驱动）、真实文件修订、多视角 file.review、稳定 issue id、范围控制、待确认 proposed patch 和确认写回防重复生成已有本地验证证据。注意：工具循环入口是 chat 自由文本，审稿 / 修订已并入循环（一次对话最多一个待确认补丁），chapter.review / 写作任务等其余显式 intent 仍走固定管线；真·LLM tool-calling headless 实跑已通过，真机 GUI 观感未验。
...
- 不能把 Agent 工具循环（含循环内审稿 / 修订）的真·LLM headless 实跑证据（单 provider）当作真机桌面端多轮渲染与补丁确认验收，也不能宣称 chapter.review / 写作任务等其余显式 intent 已工具循环化。
- 暂不承诺完整多人协作、生产级对象存储签名下载、多租户认证或全步骤 Studio 编排器。

## 8.1 当前下一步优先级

1. Agent loop 收口：chapter.review / 写作任务等剩余显式 intent 与 Q1-Q8 一致性能力逐步做成循环内工具；真机 GUI 多轮渲染与补丁确认观感随端到端复验（审稿 / 修订循环内 headless 实跑已通过）。
