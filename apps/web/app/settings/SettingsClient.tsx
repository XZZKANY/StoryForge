...
function readStoredSettings(): ProviderSettings {
  try {
    ...
    return {
      baseUrl: typeof value.baseUrl === 'string' && value.baseUrl ? value.baseUrl : defaultSettings.baseUrl,
    };
  } catch {
...
export function SettingsClient() {
  ...
  return (
    <main aria-labelledby="settings-title" className="min-h-screen bg-[#1f1f1d] px-6 py-10 text-[#e8decb]">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-[#b7afa4]">
          在这里配置兼容 OpenAI 协议的 Provider Base URL。StoryForge 会通过服务端代理访问 `/v1/models`，检测端点连通性并展示可读取的模型列表。
        </p>

        <section aria-labelledby="provider-form-title" className="mt-8 rounded-3xl border border-[#3b3a37] bg-[#2b2b29] p-6 shadow-2xl shadow-black/20">
          <h2 id="provider-form-title" className="text-lg font-semibold">Provider 连接</h2>
        ...
        <section aria-live="polite" aria-labelledby="provider-result-title" className="mt-6 rounded-3xl border border-[#3b3a37] bg-[#171715] p-6">
          <h2 id="provider-result-title" className="text-lg font-semibold">检测结果</h2>
          ...
          {models.length > 0 ? (
            <div className="mt-5">
              ...
              <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                {models.map((model) => (
                  <li key={model} className="rounded-2xl border border-[#3b3a37] bg-[#242421] px-3 py-2 text-sm text-[#ddd4c8]">
                    {model}
                  </li>
                ))}
              </ul>
