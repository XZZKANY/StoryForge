    ...
    },
    {
      ...
      content: (
        <section aria-labelledby="studio-chapter-goal-title">
          ...
          <p>当前 Web Studio 只在作品列表之后读取 {studioChapterGoalsEndpoint}，用于展示章节目标、上章摘要和连续性约束。</p>
          {chapterGoalState.status === "idle" ? (
            ...
            <dl>
              <dt>目标章节</dt>
              <dd>第 {chapterGoalState.goal.target_chapter_ordinal} 章：{chapterGoalState.goal.target_chapter_title}</dd>
              <dt>章节目标</dt>
              <dd>{chapterGoalState.goal.chapter_goal}</dd>
              <dt>上章摘要</dt>
              <dd>{chapterGoalState.goal.previous_chapter_summary ?? "暂无上章摘要"}</dd>
              <dt>连续性约束</dt>
              <dd>{chapterGoalState.goal.continuity_constraints.length === 0 ? "暂无连续性约束" : chapterGoalState.goal.continuity_constraints.join("；")}</dd>
            </dl>
          )}
        </section>
      ),
    },
    {
      ...
      content: (
        <section aria-labelledby="studio-scene-packet-title">
          ...
          <p>当前 Web Studio 只在章节目标之后读取 {studioScenePacketsEndpoint}，用于展示 Scene Packet 的证据数量和上下文预算摘要。</p>
      ...
      content: (
        <>
          <section aria-labelledby="studio-judge-review-title">
            ...
            <p>当前 Web Studio 只在 Scene Packet 之后读取 {studioJudgeReviewsEndpoint}，用于展示评审摘要、评审分数和关键问题。</p>
            {judgeReviewState.status === "idle" ? (
              ...
              <dl>
                ...
                <dt>关键问题</dt>
                <dd>{judgeReviewState.review.issues.length === 0 ? "暂无关键问题" : judgeReviewState.review.issues.map((issue) => issue.summary).join("；")}</dd>
              </dl>
            )}
          </section>
          <section aria-labelledby="studio-repair-patches-title">
            <h2 id="studio-repair-patches-title">读取 Repair 修订</h2>
            <p>当前 Web Studio 只在 Scene Packet 之后与 Judge 评审并行读取 {studioRepairPatchesEndpoint}，用于展示修订文本、差异摘要和采纳建议。</p>
          ...
          <section aria-labelledby="studio-approval-summary-title">
            ...
            <p>当前 Web Studio 只在 Repair 后读取 {studioApprovalSummaryEndpoint}，用于展示可批准对象、目标章节、回写状态和不可批准原因。</p>
            {approvalSummaryState.status === "idle" ? (
              ...
              <dl>
                ...
                <dt>可批准对象</dt>
                <dd>{approvalSummaryState.summary.approvable_object ? `${approvalSummaryState.summary.approvable_object.object_type} #${approvalSummaryState.summary.approvable_object.id}` : "暂无可批准对象"}</dd>
                <dt>目标章节</dt>
                <dd>{approvalSummaryState.summary.target_chapter ? `第 ${approvalSummaryState.summary.target_chapter.ordinal} 章：${approvalSummaryState.summary.target_chapter.title}` : "暂无目标章节"}</dd>
                <dt>回写状态</dt>
            ...
            {approvalSummaryState.status !== "ready" ? (
              ...
              <>
                ...
                <form action={approveStudioWritebackAction}>
                  ...
                  <button className="rounded-full bg-amber-700 px-5 py-2 font-semibold text-white hover:enabled:bg-amber-800 disabled:opacity-50" type="submit">
                    提交批准写回
                  </button>
                </form>
              </>
      ...
      <section aria-labelledby="studio-recovery-summary-title">
        ...
        {recoverySummaryState.status === "idle" ? (
          ...
          <dl>
            ...
            <dt>可恢复步骤</dt>
            <dd>{recoverySummaryState.summary.recoverable_steps.length === 0 ? "暂无可恢复步骤" : recoverySummaryState.summary.recoverable_steps.join("；")}</dd>
            <dt>错误摘要</dt>
