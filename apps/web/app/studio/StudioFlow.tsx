...
function getStepStatus(index: number, currentStepIndex: number, completed: boolean): StudioFlowStepStatus {
...
export function StudioFlow({ steps }: StudioFlowProps) {
  ...
  return (
    <section aria-labelledby="studio-flow-title" className="space-y-6 rounded-3xl border border-stone-200 bg-white/80 p-5 shadow-lg">
      <div>
        <h2 id="studio-flow-title" className="text-2xl font-bold text-stone-950">Studio 操作流程</h2>
        <p className="mt-2 text-sm text-stone-600">按顺序完成选作品、设目标、生成、评审并批准，当前步骤会高亮，未完成步骤会置灰。</p>
      </div>
      <ol className="grid gap-3 md:grid-cols-4" aria-label="Studio 四步操作进度">
        {steps.map((step, index) => {
          ...
          return (
            <li key={step.id} className={getStepClassName(status)} aria-current={status === "current" ? "step" : undefined}>
              <span className="text-xs font-semibold uppercase tracking-wide">{stepNumberLabels[index]}</span>
              <strong className="mt-1 block text-base">{step.label}</strong>
              <span className="mt-2 block text-sm">{step.completed ? "已完成" : status === "current" ? "当前步骤" : "等待前置步骤"}</span>
            </li>
          );
        })}
      </ol>
          ...
          return (
            ...
            >
              <div className="mb-4 flex flex-col gap-2 border-b border-stone-200 pb-4 md:flex-row md:items-start md:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-amber-800">{stepNumberLabels[index]}</p>
                  <h3 id={`studio-flow-${step.id}-title`} className="mt-1 text-xl font-bold text-stone-950">{step.title}</h3>
