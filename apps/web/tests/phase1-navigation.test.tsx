...
test("Phase 6 页面从统一数据源契约读取真实联动前置", () => {
  ...
  assertIncludesAll(registry, ["{ name: \"导出物 API\", input: \"作品 ID、章节 ID、导出类型\", output: \"artifact ID、文件名、版本、下载状态、详情和下载摘要\", status: \"Web 单点读取已实现\" }"], "Artifacts 导出物 registry 状态");
  assertIncludesAll(registry, ["{ name: \"评测运行 API\", input: \"eval set ID、模型或版本过滤\", output: \"run ID、状态、开始/结束时间、关联制品、失败样例摘要\", status: \"Web 单点读取已实现\" }"], "Evaluations 评测运行 registry 状态");
  ...
  assertIncludesAll(runs, ["失败重试执行入口", "POST /api/model-runs/job-runs/{job_run_id}/retry", "可创建恢复任务", "缺少 checkpoint 时不可重试", "Server Component 当前只展示执行契约，不伪装点击按钮"], "Runs 失败重试执行入口");
  assertIncludesAll(artifacts, ["phase6DataSources.artifacts", "数据源契约", "source.name", "source.status"], "Artifacts 数据源契约渲染");
  assertIncludesAll(artifacts, ["/api/artifacts", "制品真实读取", "Artifact #", "关联任务", "未实现边界", "制品详情与下载摘要", "payload 下载摘要", "对象存储签名 URL、指标趋势图和报告详情页仍未实现"], "Artifacts 导出物真实读取边界");
  assertIncludesAll(evaluations, ["phase6DataSources.evaluations", "数据源契约", "source.name", "source.status"], "Evaluations 数据源契约渲染");
  assertIncludesAll(evaluations, ["/api/evaluations/runs", "评测运行真实读取", "Evaluation Run #", "失败样例数量", "未实现边界", "失败样例与反馈回流", "Studio 反馈入口", "评测报告下载和复杂图表仍未实现"], "Evaluations 评测运行真实读取边界");
});
