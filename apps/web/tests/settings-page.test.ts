...
import { normalizeProviderBaseUrl, probeProviderModels } from '../app/api/provider-models/provider-models';
...
test('模型检测核心逻辑能规范化地址并提取模型', async () => {
  ...
  const fakeFetch = (async (url: URL | RequestInfo, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({ data: [{ id: 'z-model' }, { id: 'a-model' }, { bad: true }] }), {
      status: 200,
    });
  }) as typeof fetch;
