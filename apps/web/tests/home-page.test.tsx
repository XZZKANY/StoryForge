...
test('HomeShell 首屏采用 Assistant 对话台，工具流程树不默认占据首屏', () => {
  ...
  const data = read('components/home/home-data.ts');
  assert.ok(shell.includes('!w-full'), '首页 main 应覆盖全局 main 宽度避免侧栏后出现黑带');
  assert.ok(shell.includes('!m-0'), '首页 main 应覆盖全局 main 居中 margin');
  assert.ok(shell.includes('!p-0'), '首页 main 应覆盖全局 main padding');
  assert.ok(
...
test('Assistant 工具事件解析器容忍未知数据并映射真实状态', () => {
  const events = read('components/home/assistant-tool-events.ts');
  assert.ok(events.includes('parseAssistantToolEvent'), '应提供单条事件解析函数');
  assert.ok(events.includes('parseAssistantToolEvents'), '应提供事件列表解析函数');
  assert.ok(events.includes('tool_failed'), '应识别失败工具事件');
  assert.ok(events.includes('needs_approval'), '应识别需要批准事件');
  assert.ok(events.includes('book_run_progress'), '应识别 BookRun 进度事件');
  assert.ok(events.includes('return null'), '未知或畸形事件应返回 null 而不是抛错');
});
...
test('HomeShell 子页面主区域铺满右侧且移除重复状态胶囊', () => {
  ...
  assert.ok(sidebar.includes('bg-[#171715]'), '左侧侧栏背景应作为主区域颜色基准');
...
test('HomeSidebar 包含左侧主导航和账号工作区菜单', () => {
  const sidebar = read('components/home/HomeSidebar.tsx');
  const data = read('components/home/home-data.ts');
  assert.ok(sidebar.includes("'use client'"), '账号菜单点击展开需要客户端组件');
  assert.ok(sidebar.includes('useState'), '账号菜单应使用展开状态');
  ...
  assert.ok(sidebar.includes('recentItems'), '最近记录必须由上游真实数据传入');
  assert.ok(sidebar.includes('recentItems = []'), '侧栏在父组件未传数据时应回退空数组');
  assert.ok(sidebar.includes('recentItems.length > 0'), '没有真实最近记录时应展示空状态');
  assert.ok(sidebar.includes('href={item.href}'), '最近记录应渲染为可追溯链接');
  assert.ok(sidebar.includes('homeAccountMenuItems'), '侧栏应消费账号菜单数据');
  assert.ok(sidebar.includes('activeView'), '侧栏应根据 activeView 标记当前子页');
  assert.ok(sidebar.includes('createHomeViewHref'), '侧栏导航应生成 /?view=... 链接');
  assert.ok(sidebar.includes('aria-expanded={isAccountMenuOpen}'), '账号按钮应暴露展开状态');
  assert.ok(sidebar.includes('isAccountMenuOpen &&'), '账号菜单应点击后条件渲染');
  assert.ok(!sidebar.includes('Provider/API Key 等系统设置'), '账号菜单说明不应常驻占用侧栏');
  assert.ok(data.includes('Provider 待检查'), 'Provider 未配置时必须显示待检查');
  assert.ok(data.includes('Provider/API Key'), '账号弹层应暴露 Provider/API Key 入口');
  assert.ok(!sidebar.includes('Provider 正常'), '不得伪造 Provider 正常状态');
});
...
test('首页最近记录不得使用静态伪历史', () => {
  ...
  for (const fakeRecord of [
    ...
    assert.ok(!sidebar.includes(fakeRecord), `HomeSidebar 不应内置伪造最近记录：${fakeRecord}`);
  }

  assert.ok(
    page.includes('readRecentAssistantSessions'),
    '首页应读取真实 Assistant 最近会话，而不是传空数组',
  );
  assert.ok(page.includes('recentItems={recentItems}'), '首页应把真实最近记录传给 HomeShell');
  ...
  assert.ok(shell.includes('recentItems = []'), 'HomeShell 应把最近记录默认视为空数组');
...
test('HomeShell 子页承载旧页面核心功能而不是只给入口', () => {
  const shell = read('components/home/HomeShell.tsx');
  assert.ok(shell.includes('HomeProjectsPanel'), 'projects 子页应渲染本地可交互项目面板');
...
test('HomeProjectsPanel 提供本地化 Projects 交互而不是只展示假项目', () => {
  const panel = read('components/home/HomeProjectsPanel.tsx');
  assert.ok(panel.includes("'use client'"), 'Projects 面板需要客户端交互');
  assert.ok(
    panel.includes('useState<readonly HomeProject[]>(() => readLocalProjects())'),
    'Projects 应通过 state 惰性初始化读取 localStorage，避免 effect 内同步 setState',
  );
  assert.ok(!panel.includes('useEffect'), 'Projects 不应为了同步读取 localStorage 使用 effect');
  assert.ok(
    panel.includes('storyforge-home-projects'),
    'Projects 应使用独立 localStorage key 保存用户本地项目',
  );
  assert.ok(
  ...
  assert.ok(panel.includes('createProject'), 'New project 按钮应创建本地项目');
  assert.ok(panel.includes('setSortMode'), '排序按钮应切换排序模式');
  assert.ok(panel.includes('setSearchQuery'), '搜索框应更新过滤条件');
  assert.ok(panel.includes('type="button"'), '项目项应保持可点击按钮语义');
  assert.ok(panel.includes('filteredProjects'), '项目列表应由搜索过滤结果驱动');
  assert.ok(panel.includes('localStorage.setItem'), '新建项目应持久化到本地浏览器');
  assert.ok(
  ...
  assert.ok(panel.includes('onClick={createProject}'), '空状态 New project 按钮应复用真实创建逻辑');
