...
test('HomeShell 首屏采用 Assistant 对话台，工具流程树不默认占据首屏', () => {
  ...
  assert.ok(shell.includes('bg-background'), '首页内容区域应使用语义主题背景');
  assert.ok(!shell.includes('bg-[#171715]'), '首页内容区域不应硬编码黑夜模式背景');
});
...
test('HomeShell 子页面主区域铺满右侧且移除重复状态胶囊', () => {
  ...
  assert.ok(shell.includes('overflow-x-hidden bg-background'), '右侧 main 背景应与左侧侧栏一致');
...
test('UnifiedSidebar 作为全站唯一主导航和账号工作区菜单', () => {
  ...
  assert.ok(sidebar.includes('运行日志 (Runs)'), '统一侧栏应包含运行入口');
  assert.ok(sidebar.includes('全局设置'), '统一侧栏应包含设置入口');
