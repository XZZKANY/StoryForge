...
def test_skill_audit_summary_prefers_recorded_skill_runs() -> None:
    """progress 已包含真实 skill_runs 时，应优先保留记录顺序。"""
...
def test_skill_audit_summary_keeps_derived_behavior_without_recorded_skill_runs() -> None:
    """缺少真实 skill_runs 时，应继续使用阶段一派生规则。"""
    ...
    assert [skill["skill_name"] for skill in summary["chapters"][0]["skills"]] == [
        "generate",
        "judge",
        "approve",
        "memory_extract",
