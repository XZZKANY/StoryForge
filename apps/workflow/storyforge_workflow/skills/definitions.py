...
class NovelSkillRegistry:
    ...
    def default(cls) -> NovelSkillRegistry:
        """返回默认技能注册表，避免调用方直接依赖模块级变量。"""

        return DEFAULT_NOVEL_SKILL_REGISTRY

    @classmethod
    def with_genre_pack(cls, genre: str) -> NovelSkillRegistry:
        """显式加载单个题材技能包，默认链路不自动包含题材技能。"""

        normalized_genre = genre.strip().lower()
        try:
            genre_skills = _GENRE_PACKS[normalized_genre]
        except KeyError as exc:
            raise ValueError(f"未知题材技能包：{genre}") from exc
        return cls((*DEFAULT_NOVEL_SKILL_REGISTRY.list(), *genre_skills))
...
_GENRE_PACKS: dict[str, tuple[NovelSkillDefinition, ...]] = {
    "mystery": (
        _skill(
            name="clue_fairness_judge",
            description="检查悬疑章节是否公平埋设线索、误导与揭示。",
            trigger_conditions=("显式选择 mystery 题材技能包", "章节草稿已生成", "线索清单存在"),
            required_inputs=("draft_hash", "clue_map_ref", "reveal_plan_ref", "reader_knowledge_ref"),
            produced_outputs=("fairness_report_id", "clue_issue_count", "decision"),
            allowed_statuses=("pass", "repair", "awaiting_review"),
            audit_fields=("skill_name", "skill_version", "fairness_report_id", "clue_issue_count", "decision"),
            next_skills=("repair", "approve"),
        ),
    ),
    "xuanhuan": (
        _skill(
            ...
            trigger_conditions=("显式选择 xuanhuan 题材技能包", "章节草稿已生成", "战力等级表存在"),
            required_inputs=("draft_hash", "power_scale_ref", "character_state_ref", "battle_context_ref"),
            produced_outputs=("power_scale_report_id", "scale_issue_count", "decision"),
            allowed_statuses=("pass", "repair", "awaiting_review"),
            audit_fields=("skill_name", "skill_version", "power_scale_report_id", "scale_issue_count", "decision"),
            next_skills=("repair", "approve"),
        ),
    ),
    "romance": (
        _skill(
            ...
            trigger_conditions=("显式选择 romance 题材技能包", "章节草稿已生成", "关系弧规划存在"),
            required_inputs=("draft_hash", "relationship_arc_ref", "character_boundary_ref", "emotion_beats_ref"),
            produced_outputs=("relationship_report_id", "arc_issue_count", "decision"),
            allowed_statuses=("pass", "repair", "awaiting_review"),
            audit_fields=("skill_name", "skill_version", "relationship_report_id", "arc_issue_count", "decision"),
            next_skills=("repair", "approve"),
        ),
    ),
}
