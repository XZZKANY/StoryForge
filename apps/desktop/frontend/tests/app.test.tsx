...
test('编辑器页签和编辑面板保持 tabpanel 语义关联', () => {
  assert.match(appShellSource, /id="editor-panel"/);
  assert.match(appShellSource, /role="tabpanel"/);
  assert.match(appShellSource, /aria-labelledby=/);
  assert.match(assistantPanelSource, /id="assistant-panel"/);
});
...
test('App 切换文件保留多标签 buffer，仅关闭或离开项目时确认 dirty', () => {
  ...
  assert.match(editorTabsSource, /leaveProject\('切换项目'/);
