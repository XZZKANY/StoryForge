...
test('dirty editor projection records the file that actually became dirty', () => {
  assert.equal(nextDirtyEditorFile(null, 'D:\\Book\\正文\\第01章.md', true), 'D:\\Book\\正文\\第01章.md');
});
...
test('dirty editor projection only clears the matching clean file', () => {
  ...
  assert.equal(
    nextDirtyEditorFile(dirtyFile, 'D:\\Book\\正文\\第02章.md', false),
    dirtyFile,
  );
});

test('dirty editor guard allows pinning or reopening the same file without a discard prompt', () => {
  const dirtyFile = 'D:\\Book\\正文\\第01章.md';

  assert.equal(shouldConfirmBeforeReplacingDirtyEditor(dirtyFile, dirtyFile), false);
});

test('dirty editor guard prompts before replacing the dirty buffer with another target or settings', () => {
  const dirtyFile = 'D:\\Book\\正文\\第01章.md';
