...
test('matchEntityIdsInLine 按 canonical_name 与别名做包含匹配，单字符表面形跳过', () => {
  const data = mapObservatoryPayload(STRUCTURED_PAYLOAD, new Set());
  const entities = [
    ...data.entities,
    {
  ...
  assert.deepEqual(matchEntityIdsInLine(entities, '林砚举起铜灯照向巷口。'), ['item_lamp']);
  assert.deepEqual(matchEntityIdsInLine(entities, '那盏提灯还亮着。'), ['item_lamp']);
  // 「刀」是单字符表面形：包含匹配噪声过大，跳过不亮。
  assert.deepEqual(matchEntityIdsInLine(entities, '他握紧刀。'), []);
  assert.deepEqual(matchEntityIdsInLine(entities, '与实体无关的一行。'), []);
  assert.deepEqual(matchEntityIdsInLine(entities, ''), []);
});
