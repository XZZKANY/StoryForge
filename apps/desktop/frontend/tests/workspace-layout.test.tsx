...
} from '../src/components/shell/useWorkspaceSidePanelLimit';
import { workspacePrimaryMinWidth, workspaceSidePanelLimit } from '../src/lib/workspace-layout';
...
test('平衡态为正文和 Agent 留出空间，聚焦态释放右栏占用', () => {
  ...
  expect(workspacePrimaryMinWidth(1024)).toBe(420);
  expect(workspacePrimaryMinWidth(900)).toBe(332);
  expect(workspacePrimaryMinWidth(800)).toBe(232);
});
...
test('compact 响应式边界在 899px 启用，900px 退出', () => {
  let width = 900;
  vi.spyOn(window, 'innerWidth', 'get').mockImplementation(() => width);
  function Harness() {
    return <output>{useCompactWorkspaceViewport() ? 'compact' : 'desktop'}</output>;
  }
  const container = document.createElement('div');
  document.body.appendChild(container);
  const root = createRoot(container);
