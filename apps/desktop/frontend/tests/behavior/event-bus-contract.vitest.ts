...
// ①DOM CustomEvent 桥（编辑器 ↔ 对话协调）②WS 消息守卫（后端帧解码）③F10 断线重建。
...
describe('flushActiveEditorToDisk：读盘前请编辑器落盘的请求/应答握手', () => {
  ...
  it('编辑器保存失败时 reject，阻断后续读盘', async () => {
    const onRequest = () => {
      window.dispatchEvent(
        new CustomEvent(SAVE_ACTIVE_FILE_DONE_EVENT, {
          detail: { filePath: '第二章.md', status: 'error', message: '磁盘写入失败' },
        }),
      );
    };
    window.addEventListener(REQUEST_SAVE_ACTIVE_FILE_EVENT, onRequest);
    await expect(flushActiveEditorToDisk('第二章.md', 1000)).rejects.toThrow('磁盘写入失败');
    window.removeEventListener(REQUEST_SAVE_ACTIVE_FILE_EVENT, onRequest);
  });

  it('无编辑器应答时超时 reject（不再读取可能过期的磁盘内容）', async () => {
    vi.useFakeTimers();
    ...
    await expect(pending).rejects.toThrow('等待当前文件保存超时');
  });
});

describe('WS 消息守卫：后端帧的判别式（agent-socket.ts isAgent*）', () => {
