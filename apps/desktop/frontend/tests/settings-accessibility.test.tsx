...
const { probeProviderHealthMock, saveDesktopLlmConfigMock } = vi.hoisted(() => ({
  probeProviderHealthMock: vi.fn(),
  saveDesktopLlmConfigMock: vi.fn(),
}));

vi.mock('../src/components/Editor', () => ({ Editor: () => null }));
vi.mock('../src/lib/desktop-llm-config', () => ({
  getDesktopLlmConfig: vi.fn(async () => null),
  saveDesktopLlmConfig: saveDesktopLlmConfigMock,
}));
vi.mock('../src/lib/api-client', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../src/lib/api-client')>()),
  probeProviderHealth: probeProviderHealthMock,
}));
...
test('模型探测和连接测试状态对辅助技术可见', async () => {
  const { container } = await openSettings();
  saveDesktopLlmConfigMock.mockResolvedValueOnce(null);
  probeProviderHealthMock.mockReturnValue(new Promise(() => undefined));

  await click(required<HTMLButtonElement>(container, '[data-testid="provider-detect-models"]'));
  const detectStatus = required(container, '[data-testid="provider-detect-status"]');
  assert.equal(detectStatus.getAttribute('role'), 'status');
  assert.equal(detectStatus.getAttribute('aria-live'), 'polite');
  assert.equal(detectStatus.getAttribute('aria-busy'), 'true');

  await click(required<HTMLButtonElement>(container, '[data-testid="provider-health-probe"]'));
  const probeStatus = required(container, '[data-testid="provider-health-status"]');
  assert.equal(probeStatus.getAttribute('role'), 'status');
  assert.equal(probeStatus.getAttribute('aria-live'), 'polite');
  assert.equal(probeStatus.getAttribute('aria-busy'), 'true');
});
