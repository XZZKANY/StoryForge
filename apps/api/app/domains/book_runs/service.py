...
from app.domains.timeline.service import create_timeline_event, list_timeline_events
...
def _sync_completed_chapter_timeline_events(session: Session, book_run: BookRun, progress: dict) -> None:
    ...
    for completed_chapter in completed_chapters:
        if not isinstance(completed_chapter, dict):
            continue
        chapter = _chapter_from_completed_progress(session, book_run, completed_chapter)
        if chapter is None:
            continue
        if _timeline_event_exists_for_completed_chapter(session, book_run.id, chapter.id, completed_chapter):
            continue
        create_timeline_event(
            session,
            _timeline_event_payload_for_completed_chapter(book_run, progress, completed_chapter, chapter),
        )


def _chapter_from_completed_progress(
    session: Session,
    book_run: BookRun,
    ...
    if chapter_id is not None:
        ...
        if chapter is not None and chapter.book_id == book_run.book_id:
            return chapter
        return None
    ...
    return session.scalar(
        select(Chapter).where(
            ...
            Chapter.ordinal == chapter_index,
        )
    )


def _timeline_event_exists_for_completed_chapter(
    session: Session,
    book_run_id: int,
    chapter_id: int,
    completed_chapter: dict,
    ...
    candidates = session.scalars(
        select(TimelineEventRecord).where(
            TimelineEventRecord.chapter_id == chapter_id,
        )
    ).all()
    for event in candidates:
        ...
        if payload.get("source") != source_ref or not isinstance(completed_payload, dict):
            continue
        if completed_payload.get("chapter_index") == chapter_index:
            return True
    return False
