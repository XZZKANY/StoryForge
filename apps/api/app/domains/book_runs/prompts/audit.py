...
def derive_book_run_skill_chain(book_run_id: int, status: str, progress: Mapping[str, Any]) -> dict[str, Any]:
    ...
    projection = derive_skill_chain_projection(book_run_id, status, progress)
...
def _to_jsonable(value: object) -> Any:
    ...
    if is_dataclass(value) and not isinstance(value, type):
        return {field_.name: _to_jsonable(getattr(value, field_.name)) for field_ in fields(value)}
    if isinstance(value, Mapping):
