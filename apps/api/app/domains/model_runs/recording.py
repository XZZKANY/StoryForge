...
def create_model_run(session: Session, payload: ModelRunCreate) -> ModelRun:
    ...
    model_run_data = payload.model_dump()
    model_run_data["payload"] = redact_sensitive(model_run_data["payload"])
    for field in ("input_summary", "output_summary", "error_message"):
        value = model_run_data.get(field)
        if isinstance(value, str):
            model_run_data[field] = redact_sensitive_text(value)
    model_run = ModelRun(**model_run_data)
