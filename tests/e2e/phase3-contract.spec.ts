...
test("Phase 3 后端测试源码保留当前能力与退役边界证据", () => {
  ...
  assertSourceEvidence(apiTests.collaboration, ['"/api/collaboration/comments"', '建议加强旧伤带来的动作限制', '404']);
  assertSourceEvidence(apiTests.commercial, ['"/api/commercial/workspaces/', 'monthly_job_limit', '404']);
...
test("Phase 3 前端边界保留 Provider Gateway 诊断入口", () => {
  ...
  assertSourceEvidence(webSources.providers, ["Provider Gateway 模型接入层", "LLM", "Embedding", "Reranker", "图片生成或封面生成能力"]);
  assertNoSourceEvidence(
    webSources.home,
    ["/workspace", "/collaboration", "/commercial", "/analytics", "Workspace Hub 团队工作区", "Commercial Controls 商业化控制"],
  );
});
