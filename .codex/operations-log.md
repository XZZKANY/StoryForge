...
- pnpm.cmd run test：通过，Web 7/7、shared tsc --noEmit、API 147/147、workflow 13/13 全部通过。
...
P2-P7 增量文件通过局部 tsc 与测试验证，但未在最终阶段执行根级 pnpm lint，导致 ESLint/Prettier 门禁暴露格式与未使用导入问题。
...
- 确认不重复造轮子：已搜索 gzip、bundle、baseline、performance，现有实现只有 SSR 性能预算，没有 /ide 构建 bundle gzip 脚本。
...
- 已搜索 gzip、bundle、baseline、TTI、.next 等关键字，确认此前没有 /ide/page 首屏 chunk gzip 报告脚本。
...
- bundle.status=pass 是基于 Next pp-build-manifest.json 中 /ide/page 首屏 chunk 的 gzip 合计，满足 <600KB 目标。
- ttiProxy 是本地构建清单读取代理指标，报告中已明确“不等同真实浏览器 TTI”；master plan 中真实浏览器 /ide TTI 仍需要 Playwright/浏览器性能采样补证。
...
- 将遵循命名约定：Python snake_case、中文 docstring、测试函数 test_*。
- 将遵循代码风格：ruff/isort、类型标注、报告写入项目 .codex/。
- 确认不重复造轮子：已搜索 p95、latency、baseline、perf_counter，当前只有 middleware latency 日志和 DB timeout 测试，没有 IDE SSE p95 基线脚本。
...
- 命名约定：新增 Python 函数使用 snake_case，测试函数以 test_ 开头。
- 代码风格：标准库、第三方、项目模块分组导入；通过 ruff 检查。
- 文件组织：API 脚本位于 pps/api/scripts/，测试位于 pps/api/tests/，报告写入 .codex/。

### 3. 对比了以下相似实现

- test_ide_run_events.py：本轮继续验证同一 SSE 端点，但新增 p95 延迟和报告输出。
- test_book_runs.py：本轮复用 BookRun 创建与 progress 回填模式，不重新造数据初始化逻辑。
- measure-ide-build-budget.mjs：本轮沿用 .codex/*baseline.json 思路，但用 Python/TestClient 覆盖后端 SSE。

### 4. 未重复造轮子的证明

- 已搜索 p95、latency、baseline、perf_counter；此前只有中间件日志和 DB 超时测试，没有 IDE SSE p95 基线报告。
...
□ 将遵循命名约定：Python snake_case、pytest test_...、中文 docstring。
...
- 命名约定：新增函数使用 _execute_judge_*_command snake_case；测试命名使用 test_...。
...
2. 在 JudgeRepairWorkbench.tsx 导出并实现 buildJudgeRepairCommandArgs、
...
- 命名约定：新增 buildJudgeRepairCommandArgs、
esolveJudgeRepairResult、
...
- Web 类型检查：pnpm --filter @storyforge/web lint，tsc --noEmit 通过。
- 仓库 lint：pnpm lint，ESLint 与 Prettier 均通过。
...
- 代码风格：保留函数组件和
ode:test 断言方式
- 文件组织：前端控件仍位于 components/ide/personalization，导航契约仍位于 tests/phase1-navigation.test.tsx
...
- test_ide_run_events.py：后端 SSE 事件契约与本地 smoke 的事件形态一致
...
- 独立审计脚本：actual_chars=316586，目标 200000，bad_markers=[]，文件大小 968394 bytes。
...
时间：2026-05-30 04:05:22

□ 已查阅上下文摘要文件：.codex/context-summary-novel-quality-total.md
□ 将使用以下可复用组件：

- NarrativeContext: apps/workflow/storyforge_workflow/prompts/models.py - prompt 聚合输入
- StyleDirective: apps/workflow/storyforge_workflow/prompts/models.py - 文风约束和目标指标
- PacingDirective: apps/workflow/storyforge_workflow/prompts/models.py - 节奏约束
- NovelLoopPorts: apps/workflow/storyforge_workflow/orchestrators/novel_loop.py - 静态检查端口注入
- NovelLoopResult: apps/workflow/storyforge_workflow/orchestrators/novel_loop.py - 保持 BookLoop 状态契约
- _section()/_join_sections(): apps/workflow/storyforge_workflow/prompts/builder.py - prompt 分段渲染

□ 将遵循命名约定：Python snake_case/PascalCase dataclass；TypeScript camelCase/PascalCase 类型。
□ 将遵循代码风格：冻结 dataclass、纯函数 prompt builder、pytest 字符串契约、TSX 服务端渲染测试。
□ 确认不重复造轮子，证明：检查了 prompts、orchestrators、exports、book-runs 审计组件，未发现已有静态小说质量检查模块或质量摘要模型。

## 工具链替代记录

时间：2026-05-30 04:05:22

- sequential-thinking、shrimp-task-manager、desktop-commander、context7、github.search_code 在当前工具列表不可用。
- 替代方式：使用本地文件扫描、pytest 基线验证、.codex/context-summary-novel-quality-total.md 和 update_plan 记录决策。
- 基线验证：uv run pytest tests/test_prompt_builder.py -v 通过 14 项；uv run pytest tests/test_novel_loop_single_chapter.py -v 通过 4 项。

## 阶段记录 - Task 1-3 质量底座

时间：2026-05-30 04:07:37

### 1. 复用了以下既有组件

- NarrativeContext: 增加 scene_quality_plan 字段，继续作为 builder 主输入。
- _str() / _str_list(): 用于 scene_quality_plan state 映射。
- _section() / _join_sections(): 用于 【场景质量计划】 空段省略渲染。

### 2. 遵循了以下项目约定

- 命名约定：新增 SceneQualityPlan、QualityScore、QualityIssue、QualityReport、RevisionStrategy 均为 PascalCase frozen dataclass。
- 代码风格：新增逻辑保持纯函数、无真实 LLM 依赖、空数据可退化。
- 文件组织：模型在 prompts/models.py，state 适配在 prompts/context.py，prompt 渲染在 prompts/builder.py。

### 3. 对比了以下相似实现

- StyleDirective.has_content()：SceneQualityPlan.has_content() 沿用同类空段判断。
- _pacing_from_state()：_scene_quality_plan_from_state() 沿用 Mapping 校验和安全退化。
- _pacing_section()：_scene_quality_section() 沿用逐字段组行后统一 _section() 渲染。

### 4. 未重复造轮子的证明

- 检查了 prompts 与 orchestrators，无已有质量计划或质量评分模型。
- 本次新增模型是跨 prompt、静态检查、NovelLoop 和审计的共享契约，避免各层重复定义。

### 5. 验证结果

- 红灯：uv run pytest tests/test_prompt_builder.py::test_draft_prompt_injects_scene_quality_plan tests/test_prompt_builder.py::test_context_from_state_maps_scene_quality_plan -v 初始失败，提示 SceneQualityPlan 不存在。
- 修复中发现一次 PowerShell 字符串替换把 ` 
 ` 字面量写入 __init__.py，已立即定位并修正。
- 绿灯：uv run pytest tests/test_prompt_builder.py -v 通过 17 项。

## 阶段记录 - Task 4-8 静态检查与修订闭环

时间：2026-05-30 04:16:31

### 1. 复用了以下既有组件

- build_critique_prompt() / build_revision_prompt()：在原有 prompt builder 内增强契约，不新增独立 prompt 系统。
- NovelLoopPorts：新增可选 check_static_quality 端口，默认空实现保持旧测试和调用方可退化。
- pytest 字符串契约：新增测试与既有旧契约测试同时通过。

### 2. 遵循了以下项目约定

- 静态检查位于 storyforge_workflow/quality 独立模块，避免和 LLM Judge 耦合。
- StaticProseIssue 使用 frozen dataclass，输出字段可直接序列化进入 repair report。
- 黄金样例放在 tests/fixtures/quality_cases，测试通过 deterministic 规则回归。

### 3. 对比了以下相似实现

- NovelLoopPorts.extract_memory 默认空端口：check_static_quality 沿用同样的可选端口模式。
- build_revision_prompt() 原清单注入：新增分级策略段后仍保留完整正文输出契约。
- build_critique_prompt() 原通过哨兵：新增 DECISION/SCORE/ISSUE 后仍保留“通过”和旧格式兼容说明。

### 4. 未重复造轮子的证明

- 检查 workflow 未发现既有 deterministic 小说坏味道检查模块。
- 没有引入外部 NLP 或真实 LLM 依赖，避免新增维护面和测试不稳定性。

### 5. 失败补救

- PowerShell here-string 解析错误导致首个测试写入脚本未执行；改为分段写入。
- Python stdin 管道出现中文编码污染，已用 PowerShell/.NET UTF-8 无 BOM 重写相关文件。
- 旧 prompt 契约断言失败，已补回旧格式兼容文本和中文维度词。

### 6. 验证结果

- 红灯：质量模块不存在、Prompt 新契约缺失、NovelLoop 端口缺失均按预期失败。
- 绿灯：uv run pytest tests/test_prose_static_check.py tests/test_prompt_builder.py tests/test_novel_loop_single_chapter.py -v 通过 29 项。

## 阶段记录 - Task 9-10 审计摘要与 Web 展示

时间：2026-05-30 04:23:11

### 1. 复用了以下既有组件

- export_book_run_audit_report()：追加质量摘要字段，不改变原有章节证据链。
- BookRunAuditPanel：在既有审计事件列表上增加质量摘要、章节分数、主要问题和人工审查建议。
- apps/api/tests/test_book_exporter.py、apps/web/tests/book-run-audit.test.tsx：沿用现有导出和服务端渲染测试模式。

### 2. 遵循了以下项目约定

- API 质量摘要从 BookRun.progress.completed_chapters 聚合，缺数据时输出空摘要。
- Web 无质量数据时显示“暂无质量摘要”，不报错。
- 未引入新 UI 库或新服务依赖。

### 3. 失败补救

- 初次测试编辑再次出现 ` 
 ` 字面量污染，已重写相关测试文件为 UTF-8 无 BOM。
- Web 隔离 worktree 缺少 node_modules，已执行 pnpm install --frozen-lockfile 后重跑。
- API/Web 红灯均确认是功能缺失，随后实现并通过。

### 4. 验证结果

- uv run pytest tests/test_book_exporter.py -v 通过 2 项。
- uv run pytest -v（apps/api）通过 305 项，6 个既有 warning。
- pnpm --filter @storyforge/web test -- book-run-audit.test.tsx 通过 2 项。
- pnpm --filter @storyforge/web test 通过 133 项。
...
- ?????????? `_scene_quality_section()` ? `_scene_quality_plan_from_state()` ???????????????
- ?? `prompts/__init__.py`?`prompts/context.py`????????? UTF-8 BOM??? UTF-8 ? BOM ?????
- ?? `apps/workflow/storyforge_workflow/quality/__pycache__` ?????????????????
- ?? `.codex/ide-performance-baseline.json` ??????????????????????
- ?????? `issue_count` ???????????? `top_quality_issues` ???????
- `.prettierrc.json` ?? `endOfLine: auto`??????? worktree ??? CRLF ????? Prettier ?????? Web ???

### 2. ??????

- `uv run pytest -v`?`apps/workflow`??109 passed?
- `uv run pytest -v`?`apps/api`??305 passed?6 ??? warning?JWT key length?HTTP_422 deprecated??
...
- `NarrativeContext`??? prompt ??????????????????????????????
- `_section()` / `_join_sections()`????? `????????`???????????????
- `NovelLoopPorts` / `NovelLoopResult`????? `check_static_quality` ?????????????????????
- `export_book_run_audit_report()`?????????????????????
- `BookRunAuditPanel`??????????????????????????????

#### ?????????

- Python ???? snake_case ?? / ??? PascalCase dataclass?TypeScript ?? PascalCase ??? camelCase ?????
- Workflow ????? prompt builder?????? deterministic pytest ?????
- API/Web ???? domain / route ????????????? UI ??

#### ?????????

- `StyleDirective.has_content()`?`SceneQualityPlan.has_content()` ?????????
- `_pacing_from_state()`?`_scene_quality_plan_from_state()` ?? Mapping ?????
- `NovelLoopPorts.extract_memory`?`check_static_quality` ??????????
- `auditEvents()`??????????? `formatEvidenceValue()` ??????

#### ?????????

- ??? workflow prompts?orchestrators?API exports?Web audit ???????? deterministic ??????? BookRun ???????
- ?? quality ?????????????????? LLM Judge?????? prompt ???
