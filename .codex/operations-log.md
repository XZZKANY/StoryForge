...
时间：2026-05-14 00:00:00 +08:00

### 研究与检索记录

- 已确认隔离 worktree：`C:/Users/kanye/.config/superpowers/worktrees/1-renovel-ai-ai-rag-tavern/ph2-plan`，当前分支 `ph2-plan`。
- 已确认已提交仓库中只有 Phase 1 工程计划，PH2 仅存在于总体中文规格的演进路线中；因此本次先生成 PH2 实施计划，不直接改业务代码。
- 已分析 3 个以上相似实现：资产 API、连续性与 Scene Packet、LangGraph 生成工作流、Phase1 API 闭环测试、前端静态契约测试。
- 已查询官方文档替代不可用的 Context7：FastAPI APIRouter/response_model、SQLAlchemy Declarative relationship、Pydantic Field/model_validator、LangGraph durable execution/interrupt、Next.js App Router。
- 当前会话没有 `github.search_code` 工具，无法执行 AGENTS 指定的开源代码搜索；已记录限制，计划不引用未验证的开源实现细节。
- 当前会话没有 `sequential-thinking` 和 `shrimp-task-manager` 直接工具；已使用 Codex 计划工具和本地检索模拟流程并留痕。
- `pnpm --dir <worktree> install --frozen-lockfile` 失败：本机 pnpm v11.0.9 与项目 `packageManager` pnpm@9.15.4 不一致。实施前必须修正本地 pnpm 版本。

### 编码前检查 - PH2 工程计划

- 已查阅上下文摘要文件：`.codex/context-summary-ph2-plan.md`。
- 将使用以下可复用组件：
  - `Asset` 与 `EvidenceLink`：承载世界观条目、风格包和证据追溯。
  - `ContinuityRecord` 与 `ScenePacket`：承载系列级记忆和生成上下文扩展。
  - `JobRun`：承载批量精修任务状态。
  - `JudgeIssue` 与 `RepairPatch`：承载质量看板和批量精修统计。
  - `InMemoryWorkflowStore` 与 LangGraph 图模式：承载批量精修可恢复工作流。
- 将遵循命名约定：Python snake_case、类名 PascalCase、React 组件 PascalCase、页面路径 kebab-case。
- 将遵循代码风格：router/schema/service/model 分层，测试说明和文档使用简体中文。
- 确认不重复造轮子：PH2 不新增独立真相源；世界观与风格包优先复用资产谱系，质量看板优先聚合既有表。

### 编码后声明 - PH2 工程计划

#### 1. 复用了以下既有组件

- `apps/api/app/domains/assets/*`：版本化资产、资产历史和证据链接模式。
- `apps/api/app/domains/continuity/*`：连续性记录与 Scene Packet 模式。
- `apps/workflow/storyforge_workflow/*`：可恢复 LangGraph 与审计检查点模式。
- `apps/api/tests/*`：SQLite 内存库与 TestClient 本地闭环测试模式。
- `apps/web/tests/phase1-navigation.test.tsx`：前端源码契约测试模式。

#### 2. 遵循了以下项目约定

- 文档、日志、测试描述均使用简体中文。
- 工作文件写入项目本地 `.codex/` 与 `docs/superpowers/plans/`。
- 计划保持模块化单体，不引入微服务或新增重依赖。

#### 3. 对比了以下相似实现

- Phase1 资产 API：PH2 世界观与风格包沿用版本谱系，不覆盖历史。
- Phase1 Scene Packet：PH2 记忆和世界观作为固定槽位输入，不让检索片段替代真相源。
- Phase1 LangGraph：PH2 批量精修沿用 interrupt/checkpointer/thread_id 恢复模式。
- Phase1 前端契约测试：PH2 前端先做本地源码契约测试，不新增浏览器依赖。

#### 4. 未重复造轮子的证明

- 已检索 `series`、`worldbuilding`、`style_pack`、`batch_refinement`、`quality dashboard` 等关键词，当前仓库没有 PH2 领域实现。
- 计划明确复用 `Asset`、`ContinuityRecord`、`JobRun`、`JudgeIssue`、`RepairPatch`，只新增缺失的系列、应用关系和聚合接口。
...
- 新增 `apps/api/tests/test_phase2_domain_schema.py` 后运行：`uv run pytest tests/test_phase2_domain_schema.py -q`。
- 结果：失败，`ModuleNotFoundError: No module named 'app.domains.series'`，符合“系列领域模型尚不存在”的预期。

### 实施记录

- 新增 `apps/api/app/domains/series/models.py` 与 `__init__.py`。
- 修改 `apps/api/app/domains/books/models.py`，为 `Book` 增加 `series_links`、`series_memory_snapshots`、`style_pack_applications` 关系。
- 修改 `apps/api/app/models.py` 注册 Phase 2 模型。
- 新增迁移 `apps/api/alembic/versions/20260514_phase2_创建_phase_2_领域模型.py`。
- 发现既有迁移 `9f2b3c4d5e6f_为资产增加版本谱系键.py` 在 SQLite 下使用 `ALTER COLUMN SET NOT NULL` 导致本地迁移验证失败；根因是 SQLite 不支持该语法，已改为 Alembic `batch_alter_table`。

### 验证记录

- `uv run pytest tests/test_phase2_domain_schema.py tests/test_domain_schema.py -q`：通过，`13 passed`。
- 使用临时 SQLite `DATABASE_URL` 运行 `uv run alembic upgrade head`：通过，依次升级到 `71dfabf6badf`、`9f2b3c4d5e6f`、`20260514_phase2`。
- `python -m compileall apps/api/app apps/api/tests`：通过。

### 编码后声明 - PH2 Task 1

#### 1. 复用了以下既有组件

- `Base`、`IdMixin`、`TimestampMixin`、`VersionMixin`：统一实体结构。
- `Book`、`Asset`、`ContinuityRecord`、`JobRun`：作为 Phase 2 系列、记忆和风格包应用的外键目标。
- `test_domain_schema.py` 模式：复用元数据、关系和独立 mapper 配置测试方式。

#### 2. 遵循了以下项目约定

- Python 模块使用 snake_case，模型类使用 PascalCase。
- 模型 docstring 和测试说明使用简体中文。
- 新模型注册到 `app.models`，保证 Alembic 读取完整元数据。

#### 3. 对比了以下相似实现

- `Asset` 与 `ContinuityRecord`：PH2 版本化实体沿用 `VersionMixin` 和 JSON payload。
- `Book` 关系链：PH2 新增反向关系时沿用 `relationship(back_populates=...)`。
- `JobRun`：PH2 记忆快照保留任务来源外键，为后续批量精修审计做准备。

#### 4. 未重复造轮子的证明

- 检索确认仓库此前没有 `series` 领域模型。
- 新增模型只补齐系列、记忆快照和风格包应用关系，不复制资产、连续性或任务运行已有职责。

## PH2 Task 2：系列记忆与世界观 API

时间：2026-05-14 00:00:00 +08:00

### TDD 红灯记录

- 新增 `apps/api/tests/test_series_worldbuilding_api.py` 后运行 `uv run pytest tests/test_series_worldbuilding_api.py -q`。
- 结果：失败，`/api/series` 与 `/api/worldbuilding/entries` 返回 404，符合“路由尚不存在”的预期。
- 第二轮失败由 `AssetUpdate` 的显式 `None` 触发，说明世界观更新服务不能把未提供字段直接传给更新契约，必须复用 `exclude_unset=True`。

### 实施记录

- 新增 `apps/api/app/domains/series/schemas.py`、`service.py`、`router.py`、`__init__.py`。
- 新增 `apps/api/app/domains/worldbuilding/__init__.py`、`schemas.py`、`service.py`、`router.py`。
- 修改 `apps/api/app/main.py` 注册 `series_router` 和 `worldbuilding_router`。
- 修改 `apps/api/app/domains/books/models.py` 和 `apps/api/app/models.py`，使系列关系能被 SQLAlchemy 和 Alembic 统一识别。
- 新增 `apps/api/tests/test_series_worldbuilding_api.py`，覆盖系列创建、作品绑定、世界观条目版本、系列记忆快照和摘要查询。

### 验证记录

- `uv run pytest tests/test_series_worldbuilding_api.py tests/test_assets_api.py -q`：通过，`14 passed`。
- `pnpm openapi`：通过，生成 `packages/shared/src/contracts/storyforge.openapi.json`，其中包含 `/api/series`、`/api/series/{series_id}/memory-summary`、`/api/worldbuilding/entries` 等条目。
- `python -m compileall apps/api/app apps/api/tests`：通过。
...
- `Asset`：世界观条目复用版本化资产谱系。
- `Book`：系列作品绑定和世界观条目归属校验的作品根实体。
- `Series`、`SeriesBook`、`SeriesMemorySnapshot`：系列记忆领域模型。
- `AssetCreate`、`AssetUpdate`、`create_asset()`、`update_asset()`、`list_assets()`：世界观条目的版本化与列表能力。

#### 2. 遵循了以下项目约定

- 路由层只做协议和异常转换，业务规则进入 service。
- Pydantic schema 和响应模型均使用简体中文 docstring。
- 世界观条目不新增独立真相源，复用资产谱系存储。

#### 3. 对比了以下相似实现

- `assets.router`：复用同样的 `APIRouter` + `Depends(get_session)` 模式。
- `assets.service.update_asset()`：世界观更新沿用复制最新版本的模式，保留历史。
- `scene_packets.service`：系列记忆摘要和世界观列表的聚合思路保持“先聚合、后响应”的边界。

#### 4. 未重复造轮子的证明

- `worldbuilding` 只是 `Asset` 的受控子类型，不引入新的资产真相源。
- `series` 只负责系列归属、记忆快照和摘要聚合，不与生成、评审或修复逻辑耦合。

## PH2 Task 3：风格包复用 API

时间：2026-05-14 00:00:00 +08:00

### TDD 红灯记录

- 新增 `apps/api/tests/test_style_packs_api.py` 后运行 `uv run pytest tests/test_style_packs_api.py -q`。
- 结果：失败，`/api/style-packs` 返回 404，符合“路由尚不存在”的预期。
- 首次绿灯验证发现更新风格包时丢失旧 payload 中的 `voice`，测试失败并定位到 `update_style_pack()` 直接覆盖 payload。

### 实施记录

- 新增 `apps/api/app/domains/style_packs/__init__.py`、`schemas.py`、`service.py`、`router.py`。
- 修改 `apps/api/app/main.py` 注册 `style_packs_router`。
- 新增 `apps/api/tests/test_style_packs_api.py`，覆盖创建风格包、追加规则版本、应用到系列、应用到作品和读取生效规则。
- 修复 `update_style_pack()`，更新时先合并旧 payload，再写入新版本，避免未显式更新的风格字段丢失。

### 验证记录

- `uv run pytest tests/test_style_packs_api.py tests/test_series_worldbuilding_api.py -q`：通过，`4 passed`。
- `pnpm openapi`：通过，生成 `packages/shared/src/contracts/storyforge.openapi.json`，包含 `/api/style-packs`、`/api/style-packs/{asset_id}`、`/api/style-packs/{asset_id}/applications`、`/api/style-packs/effective-rules`。
- `python -m compileall apps/api/app apps/api/tests`：通过。
...
- `Asset`、`AssetCreate`、`AssetUpdate`：风格包本体复用版本化资产谱系。
- `create_asset()`、`update_asset()`：风格包创建与版本追加沿用既有资产服务。
- `StylePackApplication`：风格包应用记录复用 PH2 领域模型。
- `Series`、`Book`、`Scene`：用于校验风格包应用范围。

#### 2. 遵循了以下项目约定

- 路由层维持 `APIRouter` + `Depends(get_session)` 的既有模式。
- 服务层集中处理资产类型校验、应用范围校验和规则合并。
- 响应模型使用 Pydantic `BaseModel`，测试通过真实 FastAPI TestClient 覆盖 HTTP 契约。

#### 3. 对比了以下相似实现

- `worldbuilding.service`：同样复用 `Asset` 作为受控子类型真相源。
- `series.service`：同样在服务层校验实体存在并写入关系表。
- `assets.service.update_asset()`：风格包更新沿用“创建新版本而非原地覆盖”的模式。

#### 4. 未重复造轮子的证明

- 检索确认仓库此前没有 `style_packs` 领域 API。
- 风格包本体没有新增独立表，而是复用 `Asset(asset_type="style_pack")`。
- 应用关系没有新增迁移，复用已存在的 `style_pack_applications` 表。

## 编码前检查 - PH2 Task 4：批量精修 API 与服务

时间：2026-05-14 00:00:00 +08:00

□ 已查阅上下文摘要文件：`.codex/context-summary-ph2-task-4.md`
□ 将使用以下可复用组件：

- `JobRun`: `apps/api/app/domains/jobs/models.py` - 记录批量任务状态和进度。
- `ScenePacket`: `apps/api/app/domains/continuity/models.py` - 保存逐场景上下文快照。
- `create_judge_issues()`: `apps/api/app/domains/judge/service.py` - 生成结构化问题单。
- `create_repair_patch()`: `apps/api/app/domains/repair/service.py` - 生成定向修复补丁。

□ 将遵循命名约定：Python 模块 snake_case，类名 PascalCase，函数名动宾短语。
□ 将遵循代码风格：路由薄封装、服务层承载业务、测试使用真实 TestClient 和内存 SQLite。
□ 确认不重复造轮子，证明：已检查 `judge`、`repair`、`jobs`、`continuity`、`scene_packets`，批量精修只编排既有服务。
