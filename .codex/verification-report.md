...
生成时间：2026-05-21 01:05:00 +08:00

## 1. 需求字段完整性

- **目标**：重新分发子代理并推进 StoryForge 端到端闭环剩余任务，把风险从纯待办推进到可验证的最小执行入口与真实摘要读取。
- **范围**：workflow-to-api 最小真表 adapter/client、Studio 批准写回真实执行、Runs retry 恢复任务入口、Artifacts 详情/download 摘要、Evaluations 详情/失败样例、治理事实源和 OpenAPI。
- **交付物**：API、Workflow、Web 页面、中文契约测试、OpenAPI 生成物、TODO、架构契约、上下文摘要、操作日志和本验证报告。
- **审查要点**：不新增微服务、不引入全量前端 client、不夸大最小执行入口为完整交互式产品闭环。

## 2. 交付物映射

- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/workflow/storyforge_workflow/runtime/checkpoints.py`：新增 `ApiModelRunAdapter` 与显式 `api_job_run_id:int` 校验。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/api/app/domains/model_runs/service.py`：新增 workflow payload 写入 helper、JobRun 读取修复和 retry 恢复任务创建。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/api/app/domains/studio/service.py`：新增 `POST /api/studio/approve` 真实批准写回。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/api/app/domains/artifacts/service.py`：新增详情读取与 `payload_preview` 下载摘要。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/api/app/domains/evaluations/service.py`：新增运行详情、趋势摘要和失败样例反馈入口。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/web/app/runs/page.tsx`：新增失败重试执行入口契约展示。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/web/app/studio/page.tsx`、`apps/web/app/artifacts/page.tsx`、`apps/web/app/evaluations/page.tsx`：展示最小真实读取与执行入口边界。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/docs/architecture/phase6-workbench-contract.md`、`TODO.md`、`.codex/current-phase.md`：同步事实源。
- `D:/StoryForge/1-renovel-ai-ai-rag-tavern/packages/shared/src/contracts/storyforge.openapi.json`：由 `pnpm openapi` / `pnpm e2e` 刷新。

## 3. 本地验证命令与结果

```powershell
cd D:/StoryForge/1-renovel-ai-ai-rag-tavern/apps/api
uv run pytest tests/test_model_runs.py tests/test_studio_book_list_api.py tests/test_retrieval_workbench_api.py tests/test_artifacts.py tests/test_evaluations.py -q
```

结果：`39 passed in 2.40s`。
...
结果：Web 中文契约 `9 passed`；TypeScript `tsc --noEmit` 退出码 0。
...
结果：`pnpm verify` 通过 Docker、PostgreSQL、Redis、MinIO 本地检查；`pnpm openapi` 成功刷新 OpenAPI；`pnpm test` 退出码 0；`pnpm e2e` 为 14 个 TAP 子测试通过，并执行补偿验证，退出码 0。
...
结果：heads/current 均为 `20260520_0001 (head)`，`upgrade head` 退出码 0。
...
结果：退出码 0；仅有 Windows 换行提示，无空白错误。
## 4. 依赖与风险评估

- Docker Desktop 初始未运行，首次 `pnpm verify` 失败；已启动 Docker Desktop 并执行 `docker compose up -d postgres redis minio` 后复跑通过。
- Runs retry 当前只创建 queued 恢复任务，不立即续跑 workflow；Web 只展示执行契约，不伪装点击按钮。
- Studio 批准写回已真实更新章节、场景和 continuity，但页面仍不是完整交互式按钮流。
- Artifacts download 当前是 `payload_preview` 摘要，不是对象存储签名 URL。
- Evaluations 当前提供趋势摘要、失败样例和 Studio 反馈入口摘要，不是复杂图表或自动反馈执行。

## 5. 质量审查评分

- **代码质量**：92/100
  - 复用既有 domain router/service/schema 与测试夹具；model_runs 阻塞已修复，职责边界清晰。
- **测试覆盖**：91/100
  - 覆盖 API、Workflow、Web 契约、TypeScript、OpenAPI、Alembic 和本地服务门禁。
- **规范遵循**：92/100
  - 使用 sequential-thinking、shrimp-task-manager、子代理分工、本地验证和中文事实源；未引入未批准架构。
- **需求匹配**：90/100
  - 已从“待办”推进到最小执行/摘要闭环，但仍保留明确交互增强边界。
- **架构一致性**：92/100
  - 保持模块化单体和页面级读取，不新增微服务或全量前端 client。
- **风险评估**：90/100
  - 已记录 Docker 初始阻塞、payload preview、retry 非立即续跑和 UI 非完整交互等边界。
