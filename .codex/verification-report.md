...
- 当前权威架构、域清单、TODO/current-phase/editor-first plan 与 A6 runsheet 已同步。用户确认 A6“壳子新 UI 全流程”和“中文 IME”均无问题，runsheet 1.1–1.5、3.1–3.3 已标 PASS。
...
**诚实边界 / 剩余项**：
- packaged smoke 是 headless 本地 sidecar 传输证据，不替代真机 GUI SSE/REST 长响应、权限四轨、canon dossier、双开聚焦、暂停/停止和超时轮询观感验收。
- 用户本轮只明确确认壳子新 UI 全流程与中文 IME；A6 其余项、A7 blocker 修复与 `v0.1.2` tag 仍未完成，Gate A→B 尚不能宣称通过。
