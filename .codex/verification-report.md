...
**批次状态**：
- B1 `project.entity_budget_check`：done，批内门禁全绿，待本地提交。
- B2 `project.canon_delta`：待 B1 独立提交后执行。
- B3 retrieval transport：待 B2 独立提交后执行。
- B4-B6 desktop 必做批次：待前序批次全绿后按序执行。
- B7-B8 选做批次：仅在 B1-B6 全绿后按序尝试。
...
记录时间戳：2026-07-11 01:02:16 +08:00。
