...
**批次状态**：
- B1 `project.collapse_check`：done，批内门禁全绿，待本地提交。
- B2 Vitest 单 runner 收口：待 B1 独立提交后执行。
- B3 judge/story_state `llm_client` 收敛：条件批次，待 B1/B2 全绿后评估。
...
**未验证项**：B2/B3 与最终 `pnpm.cmd verify` / `pnpm.cmd e2e` 尚未执行；后续在本段继续追加命令与结果。
