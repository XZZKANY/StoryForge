...
## 2026-07-08 Desktop 编辑器长稿布局与保存边界修复

**触发问题**：用户真机截图显示在编辑器里追加大量空行后，Monaco 视口跳到 90+ 行附近，正文内容不可见，右侧/底部 UI 看起来被挤位；同时中文全角符号被 Monaco ambiguous unicode highlighter 画黄框，容易误判为内容损坏。

**修复内容**：
- `App.tsx` / `Editor.tsx` / `ChatWindow.tsx`：为编辑器中栏、Monaco 容器、右侧 Agent 栏补 `min-h-0` + `overflow-hidden`，把长稿滚动限制在 Monaco 内部，避免撑开外层 shell 或顶走状态栏。
- `useMonacoEditor.ts`：保留 `automaticLayout`，新增 `ResizeObserver` 兜底调用 `editor.layout()`；加载/Smoke 设置内容后立即 layout。
- `components/editor/options.ts` + `PatchReviewPanel.tsx`：统一 StoryForge 编辑器 Unicode 高亮策略，关闭中文写作中常见全角标点的 ambiguous character 黄框，保留 invisible character 提醒。
- `assistant-events.ts` / `Editor.tsx` / `ChatWindow.tsx`：保存握手升级为 `saved/skipped/error` 三态；保存失败或超时会 reject 并阻断 Agent/导出读盘；ChatWindow 在构建 context bundle 前先 flush 当前编辑器，避免上下文读取旧盘面。
- `Editor.handleSave` 改走 `performGuardedWriteback`：手动保存也遵守快照失败阻断写盘的红线，不再吞掉快照/保存错误。

**本地验证**：
- `Push-Location apps/desktop/frontend; node scripts/verify-unit.mjs app editor chat-window editor-options assistant-events; Pop-Location`：43 passed。
- `npm --prefix apps/desktop/frontend run typecheck`：通过。
- `npm --prefix apps/desktop/frontend run test:behavior -- tests/behavior/event-bus-contract.vitest.ts tests/behavior/writeback-guard.vitest.ts`：2 files / 20 tests passed。
- `npm --prefix apps/desktop/frontend run test`：node 单测 107 passed；Vitest 3 files / 24 tests passed。
- `pnpm.cmd lint`：退出码 0；Prettier 全部通过；保留 1 个既有非阻断 warning：`apps/desktop/frontend/src/components/Editor.tsx:326` missing dependency `handleExport`。
- Playwright/Vite 长稿布局探针：注入 120 行 Markdown 后，`desktop-shell.height=760`、`document.scrollHeight=760`、`editorPanel.bottom=734`、`assistantPanel.bottom=734`、`statusBar.top=734`、`editorContainer.height=626`、`editorLineCount=120`，确认长稿未撑开外层页面、状态栏仍固定在底部。

**未验证项**：
- 未跑完整 `pnpm verify`。
- 未在 packaged Tauri 真机窗口重跑人工截图场景；本轮以浏览器真实布局几何 + 单元/行为测试覆盖。

---

## 2026-07-09 Desktop 编辑器审计增量：项目路径边界与 @context containment

**审计结论**：
- 继续沿用 Monaco，不参考/照搬 VS Code 源码；本轮真正缺口在 StoryForge 自己的编辑器边界：项目路径 containment、Agent 读盘前 flush、长稿布局和测试强度。
- 长稿布局修复已在真实 Chrome + Vite 探针中复核：注入 220 行稿件后，`documentElement.scrollHeight=900`、`innerHeight=900`、`editorPanel.bottom=874`、`assistantPanel.bottom=874`、`statusBar.top=874`、`editorContainer.height=766`，确认长稿滚动锁在 Monaco 内部，没有撑开外层 shell。

**新增修复**：
- `src/lib/project/path.ts` 升级为项目路径事实源，新增 `relativePathInsideProject`、`isPathInsideProject`、`resolveProjectRelativePath`、`looksAbsolutePath`，统一处理 Windows/Unix 分隔符、大小写、`.`/`..`、空段、绝对外部路径和 sibling-prefix false positive。
- `ChatWindow` 的显式 `@context` 不再直接 `looksAbsolutePath ? 原样 : joinProjectPath` 后读盘；所有显式上下文先过 `resolveProjectRelativePath`，`@D:/outside/secret.md`、`@../secret.md`、`D:/book2` 这类输入会进入 missing context，不会读给 Agent。
- `App` 新建文件与 Agent patch 自动打开目标文件改用 centralized containment helper；`CommandPalette`、`versions`、`project index` 改用同一 helper，避免重复 `startsWith(root)` 逻辑。
- `buildProjectIndexFromEntries` 会丢弃不在 active project root 内的 file entry，给未来 Tauri/Rust 侧异常返回或 symlink 漏洞再加前端防线。

**本地验证**：
- `npm --prefix apps/desktop/frontend run typecheck`：通过。
- `npm --prefix apps/desktop/frontend run test`：node 单测 110 passed；Vitest 3 files / 24 tests passed。新增覆盖项目路径逃逸、外部绝对路径、sibling-prefix false positive、项目索引过滤外部 entry，以及 ChatWindow 显式 @context containment。
- `pnpm.cmd lint`：退出码 0；Prettier 全部通过；保留 1 个既有非阻断 warning：`apps/desktop/frontend/src/components/Editor.tsx:326` missing dependency `handleExport`。
- `git diff --check`：通过。

**未验证项 / 剩余风险**：
- 未跑完整 `pnpm verify`。
- 未在 packaged Tauri 真机窗口复跑完整打开项目 → 对话 → patch → 写回链路；本轮验证覆盖 Vite/Chrome 布局、TS 类型和前端单元/行为测试。
- Tauri/Rust raw fs commands 仍需后续从原生命令层继续做 project-root policy；本轮只收紧 Desktop frontend 调用侧 containment。

---

## 2026-07-09 Desktop 编辑器 Dirty/close/save matrix 增量

**触发清单**：`.codex/editor-vscode-grade-hardening-checklist.md` 的 P1「Dirty 状态」专项审计。前一轮已完成保存失败显式化、手动保存 guarded writeback、Agent 发送前 flush；本轮继续补「脏稿切换 / 关闭 / 预览页签」边界。

**修复内容**：
- `Editor` 新增 `onDirtyChange(filePath, dirty)` 汇报当前编辑器脏状态；关闭按钮不再自己弹确认，避免和 App 壳层重复确认。
- `App` 上提当前 dirty 文件，新增统一 discard guard。以下会替换或卸载编辑器内容的入口都会先确认：打开其他文件、预览其他文件、切项目、关闭当前文件、关闭预览文件、新建并打开文件、从预览切回固定文件、从固定文件切到预览文件、打开设置页。
- 同一路径操作不误拦：例如预览稿编辑后双击固定同一个文件，不弹「放弃修改」。
- 切同项目会话不再顺手 `selectProject(path)` 清当前文件；只有切到另一个项目才走项目切换 guard。
- 新增 `components/app/dirty-editor.ts` 纯函数，固化 dirty projection 与“是否需要确认替换 dirty buffer”的判断，避免后续页签逻辑漂移。
- `.trellis/spec/desktop/frontend/quality-guidelines.md` 新增 `Dirty Editor Navigation Guard` 可执行场景，记录 App/Editor 分工、错误矩阵和测试要求。

**本地验证**：
- `npm --prefix apps/desktop/frontend run typecheck`：通过。
- `node scripts/verify-unit.mjs dirty-editor app editor`（工作目录 `apps/desktop/frontend`）：24 passed。
- `npm --prefix apps/desktop/frontend run test`：node 单测 115 passed；Vitest 3 files / 24 tests passed。
- `pnpm.cmd lint`：退出码 0；Prettier 全部通过；保留 1 个既有非阻断 warning：`apps/desktop/frontend/src/components/Editor.tsx:332` missing dependency `handleExport`。

**未验证项 / 剩余风险**：
- 未跑完整 `pnpm verify`。
- 未在 packaged Tauri 真机窗口人工复验 dirty 后切文件 / 关文件 / 切预览 / 打开设置的弹窗观感；本轮以 App 接线护栏 + 纯函数单测 + frontend 全量测试覆盖。
- 中文 IME、撤销/重做语义、崩溃恢复、大文件性能仍是 checklist 后续切片。

---

## 2026-07-10 Desktop/API boundary hardening 收口修复

**修复内容**：
- 新增 `app.common.redaction`，在 422 validation、events、model runs、artifacts/download、timeline、retrieval、IDE audit、Agent summary 与 LLM upstream error 的写入和读取边界统一脱敏。
- Agent 相对补丁路径在发出 suggestion 前解析为项目内绝对路径；外部或 traversal 路径返回明确错误，不进入 diff confirmation。
- 文件切换会立即取消 pending autosave 并清 dirty ref；`Ctrl+,` 复用统一 dirty discard guard。
- 上下文读取改走 Rust `read_project_file(project_root, path)` canonical containment；目录索引拒绝 symlink，项目内指向外部的链接不可读给 Agent。
- Writing Run SSE 改为带 `X-StoryForge-API-Key` 的 fetch stream；Agent WS 凭据从 URL query 移到 `Sec-WebSocket-Protocol`。
- Timeline 创建校验 `project_id == book.workspace_id`；`.codex/*` 默认忽略，仅放行约定配置、hook、agent 与 verification report。

**本地验证**：
- `npm.cmd --prefix apps/desktop/frontend run typecheck`：通过。
- `npm.cmd --prefix apps/desktop/frontend run test`：Node 118 passed；Vitest 3 files / 24 tests passed。
- API 定向集合（redaction、assistant 422、events、model runs、artifacts、timeline、retrieval、IDE command/WS、LLM channel）：89 passed。
- `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml fs::tests`：11 passed，含 external path 与 symlink escape 回归。
- `uv run ruff check app ...`：通过。
- `pnpm.cmd lint`：退出码 0；保留 1 个既有非阻断 warning：`Editor.tsx` 的 `handleExport` hook dependency。
- `pnpm.cmd openapi`：生成成功；OpenAPI、Agent WS schema 与生成类型均零 drift。
- `git diff --check`：通过。

**未验证项 / 剩余风险**：
- `uv run pytest -q` 全量 API 套件分别在 4 分钟和 10 分钟命令上限超时，未返回失败用例，不能宣称 API 全量通过；本轮以 89 个相关用例覆盖改动面。
- 未在 packaged Tauri 真机窗口复跑 Agent WS、Writing Run SSE、dirty 切换和补丁确认完整 GUI 链路。
- 原始 Tauri write/delete/rename 命令仍依赖调用侧项目路径校验；本轮新增 canonical enforcement 覆盖 Agent/context 读取与 symlink 泄露主风险。

---

## 2026-07-10 S1 分支收口过闸：全量门禁实跑逮出两处漏适配并修复

**背景**：上一条「Desktop/API boundary hardening 收口修复」自述遗留「全量 API pytest 超时未跑通」。本条为过闸实跑记录（2026-07-10 规划 S1，D1-D8 拍板通过后执行）；全量门禁独立复跑、不采信自述，两处红均为本波协议/校验变更的**漏适配消费方**，零产品代码回改。

**红 1：timeline 归属校验打红 dispatch 测试（已修）**：
- `uv run pytest -q` 全量首跑（后台绕命令时限）：**4 failed / 906 passed**，四例全在 `tests/test_book_run_workflow_dispatch.py`。根因=本波新增 `_validate_timeline_scope` 要求 `book.workspace_id == project_id`，该文件老 fixture 硬编码 `project_id=1` 且 Book 未挂 workspace；`test_timeline_events.py` 本波已适配，漏改此跨域消费方。
- 修法照 timeline 测试同一模式：seed 造 `Workspace`（slug 带 uuid 防撞）→ `Book(workspace_id=workspace.id)` → `TimelineEventCreate(project_id=book.workspace_id)`。校验语义正确，不动产品代码。
- 定向复跑：`uv run pytest tests/test_book_run_workflow_dispatch.py tests/test_timeline_events.py -q` → **20 passed**；ruff 通过。

**红 2：sidecar-smoke 仍用已删除的 `?api_key=` query 连 Agent WS（已修）**：
- 第二轮 `pnpm verify` 全量 API 段绿后，daily 档冒烟在 `websocketRoundTrip` 处 403（服务端日志 `"WebSocket ...?api_key=... " 403`）——本波删除 query key 支持后，`scripts/sidecar-smoke.mjs` 未随前端一起迁子协议。
- 修法：`sidecar-smoke.mjs` 改 `new WebSocket(url, "storyforge-api-key." + base64url(key))`；顺带修同样漏适配的 headless 实跑脚本 `scripts/test-agent-websocket.py` / `scripts/test-file-revise-llm.py`（websockets `subprotocols=`，url 不再携带 key）。`Buffer` 须加进该脚本 `/* global */` 声明（eslint no-undef，第三轮 verify lint 段逮出后已修，lint 复绿 0 error）。
- 定向复跑：`pnpm smoke:sidecar` daily 档全绿（含「Agent WS 一轮通过（握手 + 收发 JSON 帧）」，实证子协议鉴权在真实起服下工作）。

**红 3：workflow `runtime/runner.py` import 排序存量红（顺手修，非本波引入）**：
- 第四轮 verify 首次推进到「Workflow Ruff 检查」段（前三轮分别死在更早的 API / lint / smoke 段）逮出 I001；workflow 树 git 干净、无 stash，系 master 存量红（该段此前未被推进到，根因未深究）。
- `uv run ruff check --fix .` 修复（2 行 import 交换），ruff 全绿 + workflow `uv run pytest -q` 323 passed 零回归。第四轮同时证明段 1-8（lint / 前端 typecheck / shared / 前端单测 / **API 全量单测** / API ruff / **sidecar smoke daily** / workflow 单测）在修复后工作树上全绿。

**已知 flaky 复现一次（非本波引入，维持观察）**：第一轮 verify 中 `test_book_generation_parallel.py::test_..._prefetches_then_revises_before_commit` 偶发红（`InvalidRequestError: Could not refresh instance '<Scene>'` 并发 session 竞争，1 failed / 909 passed，dispatch 四例修复已生效）；定向 3 连跑 12 passed × 3 全绿。该测试为登记在案 flaky（canon slice2 轮亦曾偶发），按「连红两次再立案」不立案。

**门禁实跑结果（全部独立复跑）**：
- **最终轮（第五轮）`pnpm verify` 完整档全绿（退出码 0，「所有本地核心门禁通过」）**：lint → 前端 typecheck → shared → 前端单测 → API 全量单测 → API ruff → sidecar smoke daily → workflow 单测 → workflow ruff → OpenAPI drift 十段通过。
- 全量 API：修复前首跑 4 failed / 906 passed；修复后经 verify 复跑绿（第二轮 909 passed 仅剩 flaky 一例，第四/五轮全量段直接绿）。
- `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml fs::`：11 passed（含 `read_project_file` 外部路径拒绝 + symlink 逃逸 2 新测例）。
- 前端：typecheck 干净；node 118 passed + Vitest 3 files / 24 tests passed。
- `pnpm.cmd lint`：0 error（仅既有 `Editor.tsx` handleExport warning）。
- `node scripts/check-openapi-drift.mjs`：OpenAPI 契约无漂移（Agent WS schema 与前端生成类型同步再生）。
- `cd apps/api && uv run ruff check .`：All checks passed。
- workflow：`uv run pytest -q` 323 passed；shared：`pnpm --filter @storyforge/shared test` 通过。

**F28 关账（读侧）**：蓝图 §4 F28 条目已标注——Rust `read_project_file` canonicalize containment + 目录索引拒 symlink + 前端调用侧统一收口落地；写侧 write/delete/rename 原生强制留装机前小刀（规划 S7）。

**范围说明**：`.gitignore` 白名单化 hunk 不入本 commit（拆独立 PR 挂 D3，拍板已批准方向）；编辑器 checklist 剩余切片不在本 PR 范围（IME 探针归真机波，其余转摩擦准入）。
