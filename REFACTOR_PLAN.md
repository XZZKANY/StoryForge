...
| ---- | ---- | ---- | ---- |
| B1 collapse_check | done | `e363265c` | 目标测试 25 passed；含 schema/golden 32 passed；API 全量 904 passed / 3 skipped；ruff 与 OpenAPI drift 绿。 |
| B2 vitest 收口 | done | `beaf35f6` | 24 files / 142 tests（= 118 + 24 基线），零新增 skip；typecheck 与 lint 绿；旧 runner 已删。 |
| B3 llm_client 统一 | done | `7b0b239f` | 目标域 97 passed；API 全量 907 passed / 3 skipped；ruff 绿；judge/story_state `import httpx` 归零。 |
