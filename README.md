...
- 本地闭环已验证：`pnpm verify && pnpm e2e` 通过。
- 完整测试已验证：Web 静态契约、shared 类型检查、API pytest、Workflow pytest 全部通过。
- e2e API 阶段已执行真实 FastAPI HTTP pytest，不再降级到补偿验证。
- Workflow 侧已用 OpenAI 兼容远程模型端点完成 `generate_text()` 冒烟；密钥只通过环境变量注入，不写入仓库。
- 详细证据见 `.codex/verification-report.md`。
...
- `.codex/verification-report.md` 必须记录自动化测试、页面读取、API Key 注入、Studio 批准写回、Artifacts/Evaluations 真实读取、远程 LLM 冒烟、未联通能力和 OpenAPI 变化情况。
