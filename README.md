...
- Phase 6 当前已收口：
  ...
  - Studio 页面已单点读取作品列表、章节目标、Scene Packet、Judge 评审和 Repair 修订 API。
- 下一阶段重点：
  1. Phase 7：发布与运维治理收口，检查环境样例、OpenAPI、迁移链、本地启动、发布清单、故障手册和审计状态。
  2. Phase 5：当前边界已收口；具体 workflow-to-api ModelRun 真表 adapter/client 保留为后续功能待办，本轮不新增运行时功能。
  3. Phase 6：当前边界已收口；后续在另行批准前不要继续扩 Studio/Retrieval/Runs/Artifacts/Evaluations 数据源。
...
| Phase 5 | 真实 AI/RAG 接入 | 当前边界已收口：story_memory、compiled_contexts、Workflow State 引用化、reranker 与 ModelRun 前置契约已完成；真表 adapter/client 仍是后续功能待办 |
| Phase 6 | 产品工作台可用化 | 当前边界已收口：五页入口、数据源契约、registry、Studio/Retrieval 单点读取和 Runs 后端最小契约已完成；本轮不继续扩数据源 |
