...
当前仓库已完成 Phase 1 到 Phase 4 的工程闭环，并在 Phase 5/6 中补齐了记忆、上下文、Workflow 引用化、reranker 接入和 Studio 单点真实数据读取的关键闭环。后续工作以继续打通剩余工作台数据源、真实 AI/RAG 接入收尾和发布治理为主。详细路线见 `docs/superpowers/plans/2026-05-17-storyforge-master-replan.md`。
...
- 下一阶段重点：
  1. Phase 6：继续按单页面单数据源推进 Studio Repair、批准回写、失败恢复和其他四页真实数据联动。
  2. Phase 5：补具体 workflow-to-api ModelRun 真表 adapter/client。
  3. Phase 7：发布与运维治理。
...
`.env.example` 还包含 Phase 5 真实 AI/RAG 接入的预留变量：`STORYFORGE_LLM_*`、`STORYFORGE_EMBEDDING_*`、`STORYFORGE_RERANKER_*`、`STORYFORGE_RAG_*`。这些变量当前不作为本地启动前置条件；在 provider、embedding、reranker 代码接入前，不应把它们视为真实 AI/RAG 已可用的证据。
...
| Phase 5 | 真实 AI/RAG 接入 | story_memory、compiled_contexts、Workflow State 引用化、reranker 与 ModelRun 前置契约已完成；真表 adapter/client 待继续 |
| Phase 6 | 产品工作台可用化 | 五页入口、数据源契约和 registry 已完成；Studio 已联通作品列表、章节目标、Scene Packet、Judge 评审，Repair/批准回写/失败恢复待继续 |
| Phase 7 | 发布与运维治理 | `.env.example`、迁移、启动手册、故障手册、统一验证脚本 |
