...
>
> 产品重心：StoryForge 当前转为 Desktop IDE-first。桌面端是主产品入口；Web 只保留维护、调试、兼容和契约验证职责。
>
...
- **Web 维护入口**：Next.js App Router、React、TypeScript、CodeMirror、Zustand
...
- Web 维护入口：`pnpm dev:web` 后访问 http://localhost:3000
- API：http://localhost:8000
- MinIO Console：http://localhost:9001

StoryForge 当前采用 IDE-first 产品方向：Desktop IDE 是主体验，Web 保留为维护、调试、兼容和契约验证入口。产品方向见 [`docs/architecture/ide-first-product-direction.md`](docs/architecture/ide-first-product-direction.md)。
...
pnpm dev:maintenance # 启动 API 和 Web 维护入口，并执行必要迁移
pnpm dev:api        # 只启动 API
pnpm dev:web        # 只启动 Web 维护入口
pnpm verify         # 本地核心门禁
