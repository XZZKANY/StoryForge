...
- 当前分支与远程：`master...origin/master`，本轮 `git status --short --branch` 未显示 ahead/behind；Phase 5/6 已按当前批准边界完成收口，工作区只保留本次审计文档变更，继续遵守不自动提交约束。
...
- 当前收口事实：Phase 5/6 的代码、契约、文档和验证记录已归入当前边界；本轮已推进 Runs 页面读取、Studio 批准回写/失败恢复摘要、Artifacts/Evaluations 真实摘要读取；未实现的真表 adapter/client、独立证据跳转、失败重试执行、制品下载、指标趋势和详情页仍明确保留为后续功能待办。
...
2. **Phase 5：真实 AI/RAG 依赖接入收口状态**
   ...
   - 具体 workflow-to-api ModelRun 真表 adapter/client 仍是后续功能待办，本轮不新增运行时功能。
3. **Phase 6：产品工作台可用化收口状态**
   - 当前批准边界已收口：Studio/Retrieval/Runs/Artifacts/Evaluations 的入口、契约、registry、Studio/Retrieval 单点读取、Runs 页面读取、Artifacts 导出物读取和 Evaluations 评测运行读取已完成审计校准。
...
- [ ] Workflow runtime 调用链联通：已完成 workflow 内存级 ModelRun 记录、成功/失败 sink、显式 `api_job_run_id:int` 的 API-compatible payload 映射、非法 API JobRun ID 边界测试和失败 checkpoint；剩余具体 workflow-to-api 真表 adapter/client，暂不做 HTTP client 或微服务。

### P2：Phase 6 当前边界收口 / 后续功能待办

当前说明：Phase 6 已按当前批准边界收口；静态入口、README 索引、契约文档、registry、中文契约测试、Studio/Retrieval 单点读取、Runs 页面读取、Artifacts 导出物读取和 Evaluations 评测运行读取已完成。下面未勾选项只代表后续功能待办，另行批准前不继续扩执行流。

- [x] Studio 页面串起作品、章节、Scene Packet、Judge、Repair、批准回写和失败恢复入口；已补最小中文契约入口 `作品选择`、`章节目标`、`Judge 评审`、`Repair 修订`、`批准回写`、`失败恢复`，且作品列表、章节目标、Scene Packet、Judge 评审、Repair 修订、批准回写摘要和失败恢复摘要后端 API 最小契约和 Web 单点读取已实现；批准写回执行按钮和失败续跑执行流仍待做。
...
- [x] Runs 页面展示任务状态、checkpoint、模型运行日志和失败重试；已补最小中文契约入口 `Checkpoint 状态`、`失败重试`、`ModelRun adapter 契约`，且 JobRun/checkpoint/ModelRun 摘要后端最小契约和 Web 单点读取已实现；失败重试执行仍待做。
- [x] Artifacts 页面展示导出物、上传资料、工作流快照和评测报告；已补最小中文契约入口 `导出下载`、`资料入库状态`、`快照追溯`、`报告追溯`，且导出物列表 Web 单点读取已实现；制品下载、上传资料执行、快照详情和报告详情仍待做。
- [x] Evaluations 页面展示评测集、运行记录、指标趋势和失败样例；已补最小中文契约入口 `评测集`、`运行记录`、`指标趋势`、`失败样例`，且评测运行列表 Web 单点读取已实现；评测集管理、趋势图和失败样例详情仍待做。
...
| 2026-05-20 | 四项剩余风险收口推进 | 新增 `docs/superpowers/plans/2026-05-20-four-risk-closure.md`；Runs 页面读取 `GET /api/model-runs/job-runs/1`，Studio 读取批准回写/失败恢复摘要，Artifacts 读取 `GET /api/artifacts`，Evaluations 读取 `GET /api/evaluations/runs`；registry 和前端契约测试同步为 `Web 单点读取已实现`。 | 后续只剩执行型能力：workflow-to-api 真表 adapter/client、批准写回按钮、失败续跑执行、制品下载、趋势图和详情页。 |
