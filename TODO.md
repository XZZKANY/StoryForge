...
更新时间：2026-05-20 00:40:00 +08:00
...
- 当前分支与远程：`master...origin/master`，本轮 `git status --short --branch` 未显示 ahead/behind；工作区仍有大量 Phase 5/审计文件未提交，继续遵守不自动提交约束。
...
- 当前工作区已有未提交 Phase 5/Phase 6/审计变更：包含 `.codex/operations-log.md`、`.codex/verification-report.md`、`TODO.md`、Studio/Retrieval/Runs 工作台相关代码与测试、检索/Scene Packet/ModelRun/workflow runtime 相关代码与测试、`docs/operations/alembic-validation.md`、OpenAPI 契约，以及 `story_memory`、`context_compiler`、retrieval embedding/reranker、workflow state 引用化等未跟踪新增文件。本轮用户明确要求不要自动提交。

## 3. 下一版本目标

下一版本不再重复实现 Phase 1 到 Phase 4。Phase 5/6 当前以既有收口事实为准：保留未完成能力为后续功能待办，但本轮进入 Phase 7 发布与治理收口，不继续扩 Studio/Retrieval/Runs/Artifacts/Evaluations 数据源。
...
2. **Phase 5：真实 AI/RAG 依赖接入后续待办**
   - Provider Gateway、Embedding、Reranker、Scene Packet 检索证据链、story_memory、compiled_contexts 与 Workflow State 引用化已有最小闭环。
   - 具体 workflow-to-api ModelRun 真表 adapter/client 仍待做，但本轮不新增运行时功能。
3. **Phase 6：产品工作台可用化收口状态**
   - Phase 6 全局收口和验证已经完成；Studio/Retrieval/Runs/Artifacts/Evaluations 的状态只做审计校准。
   - 后续若未另行批准，不继续扩数据源或新增工作台功能。
4. **Phase 0：同步与健康基线**
...
当前优先级：静态入口、README 索引、Phase 6 契约文档和中文契约测试已完成；下一步不要继续堆静态入口，应优先做真实数据联动闭环，并继续区分“已实现 / 已有契约但未联通 / 完全不存在 / 竞品启发”。

- [ ] Studio 页面串起作品、章节、Scene Packet、Judge、Repair、批准回写和失败恢复入口；已补最小中文契约入口 `作品选择`、`章节目标`、`Judge 评审`、`Repair 修订`、`批准回写`、`失败恢复`，且作品列表、章节目标、Scene Packet、Judge 评审、Repair 修订后端 API 最小契约和 Web 单点读取已实现；批准回写和失败恢复真实联动仍待做。
...
- [ ] Runs 页面展示任务状态、checkpoint、模型运行日志和失败重试；已补最小中文契约入口 `Checkpoint 状态`、`失败重试`、`ModelRun adapter 契约`，且 JobRun/checkpoint/ModelRun 摘要后端最小契约已实现；Runs 页面读取和失败重试仍待做。
- [ ] Artifacts 页面展示导出物、上传资料、工作流快照和评测报告；已补最小中文契约入口 `导出下载`、`资料入库状态`、`快照追溯`、`报告追溯`，真实制品数据联动仍待做。
- [ ] Evaluations 页面展示评测集、运行记录、指标趋势和失败样例；已补最小中文契约入口 `评测集`、`运行记录`、`指标趋势`、`失败样例`，真实评测数据联动仍待做。
