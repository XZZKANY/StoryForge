...
- 当前收口事实：Phase 5/6 的代码、契约、文档和验证记录已归入当前边界；未实现的真表 adapter/client、批准回写、失败恢复、独立证据跳转、Runs 页面读取、Artifacts/Evaluations 真实数据读取均明确保留为后续功能待办。本轮用户明确要求不要自动提交。
...
3. **Phase 6：产品工作台可用化收口状态**
   - 当前批准边界已收口：Studio/Retrieval/Runs/Artifacts/Evaluations 的入口、契约、registry、Studio/Retrieval 单点读取和 Runs 后端契约已完成审计校准。
...
当前说明：Phase 6 已按当前批准边界收口；静态入口、README 索引、契约文档、registry、中文契约测试、Studio/Retrieval 单点读取和 Runs 后端契约已完成。下面未勾选项只代表后续功能待办，另行批准前不继续扩数据源。
