...
- 当前分支与远程：`master...origin/master`，HEAD 为 `95f3642 feat: complete phase4 engineering and verification`。
...
- 当前工作区已有未提交文档/审计变更：`.codex/operations-log.md`、`.codex/verification-report.md` 已修改，`.codex/context-summary-storyforge-master-replan.md`、`AI_ITERATION_GUIDE.md`、`README.md`、`docs/superpowers/plans/2026-05-17-storyforge-master-replan.md` 仍为未跟踪文件。
...
- **变更收口阻碍**：当前已有未提交/未跟踪文档与审计文件，下一轮开发前必须先决定纳入版本控制、继续整理或回滚。
...
- [ ] 确认当前未提交文档/审计文件的处理策略，避免后续开发叠加在不清晰状态上。
...
- [ ] 若 Docker 可用，补跑 `pnpm verify` 并记录 PostgreSQL、Redis、MinIO 状态。

### P1：下一开发主线

- [ ] Provider Gateway 配置真实化：区分 LLM、embedding、reranker；未配置密钥时稳定回退到 deterministic provider。
...
- [ ] 补全 `.env.example`，覆盖 API、workflow、PostgreSQL、Redis、MinIO、provider、embedding、reranker 配置。
- [ ] 编写运维文档：`docs/operations/local-start.md` 已完成；`docs/operations/release-checklist.md`、`docs/operations/troubleshooting.md` 待补充。
- [ ] 加强 `scripts/verify-local.ps1`，输出清晰失败原因和下一步修复建议。
- [x] 强化 e2e 的 OpenAPI 刷新门禁：`scripts/run-e2e.mjs` 已在刷新失败时返回非零退出码，不再静默使用旧契约；`scripts/generate-openapi.ps1` 单独验证通过。
- [ ] 补齐 Alembic 从干净数据库升级到最新模型的本地验证记录。
