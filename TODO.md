...
- 当前工作区已有未提交 Phase 5/审计变更：包含 `.codex/operations-log.md`、`.codex/verification-report.md`、`TODO.md`、检索/Scene Packet/ModelRun/workflow runtime 相关代码与测试、`docs/operations/alembic-validation.md`、OpenAPI 契约，以及 `story_memory`、`context_compiler`、retrieval embedding/reranker、workflow state 引用化等未跟踪新增文件。本轮用户明确要求不要自动提交。
...
- [ ] Studio 页面串起作品、章节、Scene Packet、Judge、Repair、批准回写和失败恢复入口；已补最小中文契约入口 `作品选择`、`章节目标`、`Judge 评审`、`Repair 修订`、`批准回写`、`失败恢复`，且 `GET /api/studio/books`、`GET /api/studio/chapter-goals` 与 `GET /api/studio/scene-packets` 后端 API 最小契约和 Web 单点读取已实现；Judge、Repair、批准回写和失败恢复真实联动仍待做。
- [ ] Retrieval 页面支持资料源列表、刷新任务、搜索请求、命中预览和证据跳转；已补最小中文契约入口 `资料来源类型`、`搜索请求`、`命中预览`、`证据跳转`，真实检索数据联动仍待做。
- [ ] Runs 页面展示任务状态、checkpoint、模型运行日志和失败重试；已补最小中文契约入口 `Checkpoint 状态`、`失败重试`、`ModelRun adapter 契约`，真实数据联动仍待做。
